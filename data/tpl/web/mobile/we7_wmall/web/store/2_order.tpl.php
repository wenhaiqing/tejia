											...
											<ul class="dropdown-menu">
												...
												<?php  if($dca['order_type'] == 1) { ?>
													<?php  if($dca['status'] == 2 || $dca['status'] == 3) { ?>
														...
														<?php  if($dca['status'] == 2) { ?>
														<li><a href="<?php  echo $this->createWeburl('order', array('op' => 'status', 'id' => $dca['id'], 'status' => 3, 'type' => 'delivery_wait'));?>" onclick="if(!confirm('确定收到供应商供货?')) return false;">已收到供应商供货</a></li>
														<?php  } ?>
														<!-- <li><a href="<?php  echo $this->createWeburl('order', array('op' => 'status', 'id' => $dca['id'], 'status' => 4, 'type' => 'delivery_ing'));?>" onclick="if(!confirm('确定修改订单状态?')) return false;">设为配送中</a></li> -->
															...
															<li><a href="javascript:;" class="select-deliveryer" data-type="single" data-id="<?php  echo $dca['id'];?>">指定配送站配送</a><li>
