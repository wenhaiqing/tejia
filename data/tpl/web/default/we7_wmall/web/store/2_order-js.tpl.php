...
<script id="tpl-select-deliveryer" type="text/html">
	...
	<tr>
		...
		<td style="text-align: right">
			<a href="javascript:;" data-id="<{d[i].id}>" class="btn btn-default js-select-deliveryer">选择</a>
		</td>
	</tr>
	<{# } }>
</script>
...
$(function(){
	$(document).on('click', '.select-deliveryer', function(){
		...
		$('.js-select-deliveryer-submit').unbind().click(function(){
			...
			$.post("<?php  echo $this->createWebUrl('order', array('op' => 'set_deliveryer1'));?>", {deliveryer_id: deliveryer_id, order_ids: order_ids}, function(data){
				var result = $.parseJSON(data);
				if(result.message.errno != 0) {
			...
			$.post("<?php  echo $this->createWeburl('order', array('op' => 'status', 'status' => 4, 'type' => 'delivery_ing'))?>",{id:order_ids}, function(data){
