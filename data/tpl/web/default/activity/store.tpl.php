...
<div class="clearfix">
	<div class="alert alert-info">
		【温馨提示】：如果开启了系统卡券（卡券列表--是否使用系统卡券），就无法使用微信门店相关功能（包括：微信门店，导入微信门店，同步微信门店等）
	</div>
	<form action="" method="post" id="form2">
			...
			<div class="panel-body table-responsive">
				<table class="table table-hover" ng-controller="advAPI" style="width:100%;" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<?php  } ?>
						<td>
							<a href=" <?php  echo url('activity/store/post', array('id' => $li['id']))?>" class="btn btn-default" title="详情">编辑</a>
							<a href="<?php  echo url('activity/store/delete', array('id' => $li['id']))?>" class="btn btn-default" onclick="if(!confirm('您确定要删除吗？')) return false;" title="删除">删除</a>
						</td>
					</tr>
					<?php  } } ?>
					</tbody>
		...
		<?php  if(COUPON_TYPE ==  WECHAT_COUPON) { ?>
		<a href="<?php  echo url('activity/store/import')?>"  class="btn btn-danger col-lg-1 pull-left" style="margin-right:10px;">从微信导入</a>
		<a href="javascript:;" onclick="util.ajaxshow('<?php  echo url('activity/store/sync', array('type' => 1))?>')" class="btn btn-default pull-left" style="margin-right:10px;">更新全部门店信息</a>
		<?php  } ?>
