...
<ul class="nav nav-tabs">
	<li <?php  if($do == 'list') { ?>class="active"<?php  } ?>><a href="<?php  echo url('activity/market/list', array());?>">卡券活动列表</a></li>
	<li <?php  if($do == 'post' && !$id) { ?>class="active"<?php  } ?>><a href="<?php  echo url('activity/market/post', array());?>">添加活动</a></li>
	<?php  if($do == 'post' && $id) { ?><li class="active"><a href="<?php  echo url('activity/market/post', array('id' => $id))?>">查看活动</a></li><?php  } ?>
</ul>
<?php  if($do == 'list') { ?>
<div class="main">
		...
		<div class="table-responsive panel-body">
			<table class="table table-hover">
				...
				<tr>
					...
					<td>
						<a href="<?php  echo url('activity/market/post', array('id' => $item['id']))?>">查看详情</a>
						<a href="<?php  echo url('activity/market/delete', array('id' => $item['id']))?>" onclick="return confirm('确定删除活动吗？');return false;">删除</a>
					</td>
				</tr>
				<?php  } } ?>
				</tbody>
			...
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-12">
					<div class="panel panel-default tab-content">
						...
						<div class="panel-body">
							<span class="help-block help" id="new_member">&nbsp;&nbsp;&nbsp;新用户:  &nbsp;&nbsp;成为会员不超过一个月，并且只消费过一次或没消费的用户。<br/></span>
							<span class="help-block help" id="old_member">&nbsp;&nbsp;&nbsp;老用户:  &nbsp;&nbsp;成为会员两个月以上的用户。<br/></span>
							<span class="help-block help" id="activity_member">活跃用户: &nbsp;&nbsp;一个月内消费超过2次的用户。<br/></span>
							<span class="help-block help" id="quiet_member">沉寂用户: &nbsp;&nbsp;一个月内没有消费的用户。<br/></span>
...
<div class="form-group col-sm-12">
	<?php  if(!empty($id)) { ?>
	<a class="btn btn-primary" href="<?php  echo url('activity/market')?>">返回列表</a>
	<?php  } else { ?>
...
<script>
	...
	$('.give_member').click(function() {
		...
		$.post("<?php  echo url('activity/market/get_member_num')?>", {'type' : type, 'param' : param}, function(data) {
	...
	$('#save').click(function() {
		<?php  if(COUPON_TYPE == WECHAT_COUPON) { ?>
			...
			$.post("<?php  echo url('activity/market/checkcoupon')?>", {'coupon' : coupon}, function (data) {
				var data = $.parseJSON(data);
