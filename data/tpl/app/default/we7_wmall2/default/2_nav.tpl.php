<?php defined('IN_IA') or exit('Access Denied');?><nav class="bar bar-tab footer-bar sborder <?php  if($_GPC['do'] == 'goods') { ?>hide<?php  } ?>">
	...
	<?php  if($_W['we7_wmall2']['config']['version'] == 1) { ?>
...
<?php  if(empty($_W['fans']['follow']) && $_W['we7_wmall2']['config']['follow_guide_status'] == 1 && in_array($_GPC['do'], array('index', 'goods', 'mine'))) { ?>
<div class="follow-tips">
	<div class="info">
		<div class="logo"><img src="<?php  echo tomedia($_W['we7_wmall2']['config']['thumb']);?>" alt=""></div>
		<div class="txt"><p>欢迎进入<?php  echo $_W['we7_wmall2']['config']['title'];?><br>关注公众号，享专属服务</p></div>
	</div>
	<div class="text-btn"><a href="<?php  echo $_W['we7_wmall2']['config']['followurl'];?>">立即关注</a></div>
</div>
<?php  } ?>
