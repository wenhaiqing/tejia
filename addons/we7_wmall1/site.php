...
include 'version.php';
include 'model.php';
class We7_wmall1ModuleSite extends WeModuleSite
{
	private $cache = array();
	...
	public function checkAuth()
	{
		...
		if (!empty($_W['openid'])) {
			...
			$data = pdo_get('tiny_wmall1_members', array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
			if (empty($data)) {
				if (empty($_W['member']['uid'])) {
					...
					if (!empty($fans)) {
						...
						pdo_insert('tiny_wmall1_members', $data);
					}
				...
				if (empty($_W['member']['uid'])) {
					...
					if ($data['uid'] != $_W['member']['uid']) {
						$tables = array('tiny_wmall1_activity_coupon_grant_log', 'tiny_wmall1_activity_coupon_record', 'tiny_wmall1_address', 'tiny_wmall1_order', 'tiny_wmall1_order_comment', 'tiny_wmall1_order_current_log');
						pdo_update('tiny_wmall1_members', array('uid' => $_W['member']['uid'], 'is_sys' => 1), array('uniacid' => $_W['uniacid'], 'uid' => $data['uid']));
	...
	public function payResult($params)
	{
		global $_W;
		$config = sys_config();
		$_W['we7_wmall1']['config'] = $config;
		if ($params['result'] == 'success' && $params['from'] == 'notify' || $params['from'] == 'return' && in_array($params['type'], array('delivery'))) {
			mload()->model('order');
			$record = pdo_get('tiny_wmall1_paylog', array('uniacid' => $_W['uniacid'], 'order_sn' => $params['tid']));
			if (!empty($record)) {
				pdo_update('tiny_wmall1_paylog', array('status' => 1, 'paytime' => TIMESTAMP), array('id' => $record['id']));
			}
			$data = array('pay_type' => $params['type'], 'final_fee' => $params['card_fee'], 'is_pay' => 1, 'paytime' => TIMESTAMP);
			if ($record['order_type'] == 'order') {
				$order = pdo_fetch('SELECT id, sid, order_type, table_id, status, is_pay FROM ' . tablename('tiny_wmall1_order') . ' WHERE uniacid = :aid AND id = :id', array(':aid' => $_W['uniacid'], ':id' => $record['order_id']));
				if (!$order['is_pay']) {
					$data['transaction_id'] = $params['tag']['transaction_id'];
					$store = pdo_get('tiny_wmall1_store', array('uniacid' => $_W['uniacid'], 'id' => $order['sid']));
					...
					if ($order['order_type'] <= 2) {
						if ($store['auto_handel_order'] == 1) {
							...
							pdo_update('tiny_wmall1_order', $data, array('id' => $record['order_id'], 'uniacid' => $_W['uniacid']));
							...
							if ($store['auto_notice_deliveryer'] == 1) {
								if ($order['order_type'] == 1) {
									...
									pdo_update('tiny_wmall1_order', array('delivery_type' => $account['delivery_type'], 'status' => 3, 'delivery_status' => 3), array('uniacid' => $_W['uniacid'], 'id' => $order['id']));
									pdo_update('tiny_wmall1_order_current_log', array('delivery_type' => $account['delivery_type']), array('uniacid' => $_W['uniacid'], 'orderid' => $order['id']));
									order_insert_status_log($order['id'], $order['sid'], 'delivery_wait');
									order_deliveryer_notice($order['sid'], $order['id'], 'delivery_wait');
								}
							}
						} else {
							pdo_update('tiny_wmall1_order', $data, array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
							order_insert_status_log($order['id'], $order['sid'], 'pay');
							order_print($order['id']);
				...
				mload()->model('card');
				$order = pdo_get('tiny_wmall1_delivery_cards_order', array('id' => $record['order_id'], 'uniacid' => $_W['uniacid']));
				if (!empty($order) && !$order['is_pay']) {
					$card = pdo_get('tiny_wmall1_delivery_cards', array('id' => $order['card_id'], 'uniacid' => $_W['uniacid']));
					if (!empty($card)) {
						$member = pdo_get('tiny_wmall1_members', array('uid' => $order['uid'], 'uniacid' => $_W['uniacid']));
						if (!empty($member)) {
							$starttime = $member['setmeal_endtime'] > 0 ? $member['setmeal_endtime'] : strtotime(date('Y-m-d'));
							$endtime = strtotime("{$card['days']}days", $starttime);
							pdo_update('tiny_wmall1_delivery_cards_order', array('is_pay' => 1, 'paytime' => TIMESTAMP, 'starttime' => $starttime, 'endtime' => $endtime), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
							pdo_update('tiny_wmall1_members', array('setmeal_id' => $card['id'], 'setmeal_day_free_limit' => $card['day_free_limit'], 'setmeal_starttime' => $starttime, 'setmeal_endtime' => $endtime), array('uid' => $member['uid'], 'uniacid' => $_W['uniacid']));
				...
				$order = pdo_get('tiny_wmall1_errander_order', array('id' => $record['order_id'], 'uniacid' => $_W['uniacid']));
				if (!empty($order) && !$order['is_pay']) {
					pdo_update('tiny_wmall1_errander_order', $data, array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
	...
	protected function pay($params = array(), $mine = array())
	{
		...
		$payment = $_W['we7_wmall1']['config']['payment'];
		if (!is_array($payment)) {
