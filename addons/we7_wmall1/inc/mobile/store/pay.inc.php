...
$tables = array(
	'order' => 'tiny_wmall1_order',
	'card' => 'tiny_wmall1_delivery_cards_order',
	'errander' => 'tiny_wmall1_errander_order'
);
...
$record = pdo_get('tiny_wmall1_paylog', array('uniacid' => $_W['uniacid'], 'order_id' => $id, 'order_type' => $type));
if(empty($record)) {
	$record = array(
		'uniacid' => $_W['uniacid'],
	...
	pdo_insert('tiny_wmall1_paylog', $record);
} else {
	if($record['status'] == 1) {
...
$available_pay_types = $_W['we7_wmall1']['config']['payment'];
