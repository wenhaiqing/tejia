...
if($config['version'] == 2) {
	$url = $this->createMobileUrl('goods', array('sid' => $_W['we7_wmall1']['config']['default_sid']));
	header('location:' . $url);
	die;
}
...
if($op == 'list') {
	...
	$stores = pdo_fetchall('select id,score,title,logo,sailed,score,business_hours,is_in_business,delivery_price,delivery_free_price,send_price,delivery_time,delivery_mode,token_status,invoice_status,location_x,location_y,forward_mode,displayorder,click from ' . tablename('tiny_wmall1_store') . " where uniacid = :uniacid and status = 1", array(':uniacid' => $_W['uniacid']));
	$min = 0;
	if(!empty($stores)) {
		foreach($stores as &$da) {
			...
			$da['hot_goods'] = pdo_fetchall('select title from ' . tablename('tiny_wmall1_goods') . ' where uniacid = :uniacid and sid = :sid and is_hot = 1 limit 3', array(':uniacid' => $_W['uniacid'], ':sid' => $da['id']));
...
$_share = array(
	'title' => $_W['we7_wmall1']['config']['title'],
	'desc' => $_W['we7_wmall1']['config']['content'],
	'link' => murl('entry', array('m' => 'we7_wmall1', 'do' => 'index'), true, true),
	'imgUrl' => tomedia($_W['we7_wmall1']['config']['thumb'])
);
