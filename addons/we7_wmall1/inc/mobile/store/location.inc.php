...
if($op == 'index') {
	if($_W['member']['uid'] > 0) {
		$addresses = pdo_getall('tiny_wmall1_address', array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid']));
	}
}
...
if($op == 'suggestion') {
	...
	$type = trim($_GPC['type']);
	if($type == 'errander' && !empty($_W['we7_wmall1']['config']['errander']['city'])) {
		$query['city'] = $_W['we7_wmall1']['config']['errander']['city'];
	}
	...
	if(!empty($result['tips'])) {
		$distance_sort = 0;
		if($type == 'errander' && $_W['we7_wmall1']['config']['errander']['serve_radius'] > 0 && !empty($_W['we7_wmall1']['config']['errander']['map']['location_x']) && !empty($_W['we7_wmall1']['config']['errander']['map']['location_y'])) {
			$distance_sort = 1;
		}
		...
		foreach($result['tips'] as $key => &$val) {
			...
			if(is_array($val['location'])) {
				...
				$val['lat'] = $location[1];
				if($distance_sort == 1) {
					$val['distance'] = distanceBetween($val['lng'], $val['lat'], $_W['we7_wmall1']['config']['errander']['map']['location_y'], $_W['we7_wmall1']['config']['errander']['map']['location_x']);
					$val['distance'] = round($val['distance'] / 1000, 2);
					$val['distance_available'] = 1;
					if($val['distance'] > $_W['we7_wmall1']['config']['errander']['serve_radius']) {
