...
if($op == 'list') {
	$title = "{$_W['we7_wmall1']['config']['title']}-订单列表";
	$orders = pdo_fetchall('select a.id as aid, a.*, b.title, b.logo, b.delivery_mode, b.pay_time_limit from ' . tablename('tiny_wmall1_order') . ' as a left join ' . tablename('tiny_wmall1_store') . ' as b on a.sid = b.id where a.uniacid = :uniacid and a.uid = :uid order by a.id desc limit 15', array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid']), 'aid');
	$min = 0;
	if(!empty($orders)) {
		$order_status = order_status();
		foreach($orders as &$da) {
			$da['goods'] = pdo_get('tiny_wmall1_order_stat', array('oid' => $da['id']));
		}
...
if($op == 'more') {
	$id = intval($_GPC['id']);
	$orders = pdo_fetchall('select a.id as aid, a.*, b.title, b.logo, b,delivery_mode, b.pay_time_limit from ' . tablename('tiny_wmall1_order') . ' as a left join ' . tablename('tiny_wmall1_store') . ' as b on a.sid = b.id where a.uniacid = :uniacid and a.uid = :uid and a.id < :id order by a.id desc limit 15', array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid'], ':id' => $id), 'aid');
	$min = 0;
	if(!empty($orders)) {
		$order_status = order_status();
		foreach($orders as &$order) {
			$order['goods'] = pdo_get('tiny_wmall1_order_stat', array('oid' => $order['aid']), array('goods_title'));
...
if($op == 'cancel') {
	...
	if(!$order['is_pay']) {
		pdo_update('tiny_wmall1_order', array('status' => '6'), array('uniacid' => $_W['uniacid'], 'id' => $id));
...
if($op == 'detail') {
	$title = "{$_W['we7_wmall1']['config']['title']}-订单详情";
	$id = intval($_GPC['id']);
	$order = order_fetch($id);
	if(empty($order)) {
...
if($op == 'remind') {
	...
	$log = pdo_fetch('select * from ' . tablename('tiny_wmall1_order_status_log') . ' where uniacid = :uniacid and oid = :oid and status = 8 order by id desc',  array(':uniacid' => $_W['uniacid'], ':oid' => $id));
	...
	pdo_update('tiny_wmall1_order', array('is_remind' => '1'), array('uniacid' => $_W['uniacid'], 'id' => $id));
	message(error(0, ''), '', 'ajax');
}
