...
if($op == 'build') {
	...
	$types = array(
		'store' => array(
			'scene_str' => "we7_wmall1__store_{$sid}",
			'keyword' => "we7_wmall1__store_{$sid}",
			'name' => "{$store['title']}门店二维码",
		),
		'assign' => array(
			'scene_str' => "we7_wmall1__assign_{$sid}",
			'keyword' => "we7_wmall1__assign_{$sid}",
			'name' => "{$store['title']}排号二维码",
		),
		'table' => array(
			'scene_str' => "we7_wmall1__table_{$sid}_{$table_id}",
			'keyword' => "we7_wmall1__table_{$sid}_{$table_id}",
	...
	pdo_insert('tiny_wmall1_reply', $data);
	...
	if($type == 'store') {
		pdo_update('tiny_wmall1_store', array('wechat_qrcode' => iserializer($qrcode)), array('uniacid' => $_W['uniacid'], 'id' => $sid));
	} elseif($type == 'assign') {
		pdo_update('tiny_wmall1_store', array('assign_qrcode' => iserializer($qrcode)), array('uniacid' => $_W['uniacid'], 'id' => $sid));
	} elseif($type == 'table') {
		pdo_update('tiny_wmall1_tables', array('qrcode' => iserializer($qrcode)), array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $table_id));
