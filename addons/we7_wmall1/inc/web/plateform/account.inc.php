...
if($op == 'list') {
	...
	$list = pdo_fetchall('select * from ' . tablename('tiny_wmall1_account') . ' where uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
}

if($op == 'del') {
	$id = intval($_GPC['id']);
	pdo_delete('tiny_wmall1_account', array('uniacid' => $_W['uniacid'], 'id' => $id));
...
if($op == 'status') {
	...
	$account = pdo_get('tiny_wmall1_account', array('uniacid' => $_W['uniacid'], 'id' => $id));
	if(!empty($account)) {
		pdo_update('tiny_wmall1_account', array('status' => $status), array('uniacid' => $_W['uniacid'], 'id' => $id));
...
if($op == 'post') {
	$id = intval($_GPC['id']);
	if($id > 0){
		$sql = 'SELECT * FROM ' . tablename('tiny_wmall1_account') . " WHERE id = :id AND uniacid = :uniacid";
	...
	if(checksubmit()) {
		...
		$permission = 'we7_wmall1_menu_store';
		$permission_exist = pdo_get('users_permission', array('uniacid' => $_W['uniacid'], 'uid' => $account['uid'], 'type' => 'we7_wmall1'));
		if(empty($permission_exist)) {
			pdo_insert('users_permission', array('uniacid' => $_W['uniacid'], 'uid' => $account['uid'], 'type' => 'we7_wmall1', 'permission' => $permission));
		} else {
			pdo_update('users_permission', array('permission' => $permission), array('uniacid' => $_W['uniacid'], 'uid' => $account['uid'], 'type' => 'we7_wmall1'));
		}
		...
		if(empty($account['id'])) {
			...
			pdo_update('tiny_wmall1_account', $data, array('uniacid' => $_W['uniacid'], 'id' => $id));
		}
		message('编辑账号成功', $this->createWebUrl('ptfaccount'), 'success');
	}
	$stores = pdo_getall('tiny_wmall1_store', array('uniacid' => $_W['uniacid']), array('id', 'title'));
}
