...
global $_W, $_GPC;
$_W['page']['title'] = '短信平台-' . $_W['we7_wmall1']['config']['title'];
...
if($op == 'set') {
	if(checksubmit()) {
		...
		pdo_update('tiny_wmall1_config', array('sms' => iserializer($data)), array('uniacid' => $_W['uniacid']));
		message('短信参数设置成功', $this->createWebUrl('ptfsms'), 'success');
	}
	$sms = $_W['we7_wmall1']['config']['sms'];
}

if($op == 'record') {
	$lists = pdo_getall('tiny_wmall1_store', array('uniacid' => $_W['uniacid']), array('id', 'logo', 'title', 'sms_use_times'));
}
...
if($op == 'detail') {
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall1_sms_send_log') . $condition, $params);
	$data = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall1_sms_send_log') . $condition . ' ORDER BY id DESC LIMIT '.($pindex - 1) * $psize.','.$psize, $params);
