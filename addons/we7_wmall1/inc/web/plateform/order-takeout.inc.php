...
global $_W, $_GPC;
$_W['page']['title'] = '外卖订单-' . $_W['we7_wmall1']['config']['title'];
...
if($op == 'list') {
	...
	$wait_total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall1_order') . ' WHERE uniacid = :uniacid AND sid = :sid and status = 1', array(':uniacid' => $_W['uniacid'], ':sid' => $sid));
			...
			foreach($all_fields as $key => $li) {
				...
				if(in_array($field, array_keys($order_fields))) {
					if($field == 'sid') {
						...
						$log = pdo_fetch('select * from ' . tablename('tiny_wmall1_order_status_log') . ' where uniacid = :uniacid and oid = :oid order by id desc', array(':uniacid' => $_W['uniacid'], ':oid' => $row['id']));
