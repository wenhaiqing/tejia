...
global $_W, $_GPC;
$_W['page']['title'] = '订单列表-' . $_W['we7_wmall1']['config']['title'];
mload()->model('store');
mload()->model('order');
...
if($op == 'list') {
	...
	$wait_total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall1_order') . ' WHERE uniacid = :uniacid AND sid = :sid and status = 1 and order_type < 3', array(':uniacid' => $_W['uniacid'], ':sid' => $sid));
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall1_order') .  $condition, $params);
	$data = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall1_order') . $condition . ' ORDER BY addtime DESC LIMIT '.($pindex - 1) * $psize.','.$psize, $params);
...
if($op == 'status') {
	...
	if($status == 2) {
		$order = pdo_fetchall('select id from ' . tablename('tiny_wmall1_order') . " where uniacid = :uniacid and sid = :sid and status != 1 and id in ({$ids_temp})", $params);
	} elseif($status == 3) {
		$order = pdo_fetchall('select id from ' . tablename('tiny_wmall1_order') . " where uniacid = :uniacid and sid = :sid and status != 2 and status != 3 and id in ({$ids_temp})", $params);
	} elseif($status == 4) {
		$order = pdo_fetchall('select id from ' . tablename('tiny_wmall1_order') . " where uniacid = :uniacid and sid = :sid and status != 2 and status != 3 and id in ({$ids_temp})", $params);
	} elseif($status == 5) {
		$order = pdo_fetchall('select id from ' . tablename('tiny_wmall1_order') . " where uniacid = :uniacid and sid = :sid and status != 4 and id in ({$ids_temp})", $params);
	}
	if(!empty($order)) {
	...
	foreach($ids as $id) {
		...
		if($id <= 0) continue;
		if($status == 7) {
			pdo_update('tiny_wmall1_order', array('pay_type' => 'cash', 'is_pay' => 1), array('uniacid' => $_W['uniacid'], 'id' => $id, 'is_pay' => 0));
			...
			pdo_update('tiny_wmall1_order', $update, array('uniacid' => $_W['uniacid'], 'id' => $id));
			//如果是自提订单, 将订单状态设置为4
			pdo_update('tiny_wmall1_order', array('status' => 4), array('uniacid' => $_W['uniacid'], 'id' => $id, 'order_type' => 2, 'status' => 2));
		}
		if($status > 2) {
			pdo_update('tiny_wmall1_order_stat', array('status' => 1), array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'oid' => $id));
		}
		order_insert_status_log($id, $sid, $type);
		order_status_notice($sid, $id, $type);
		if($status == '3') {
			//设置为待配送, 并通知配送员配送
			pdo_update('tiny_wmall1_order', array('delivery_type' => $account['delivery_type']), array('uniacid' => $_W['uniacid'], 'id' => $id));
			pdo_update('tiny_wmall1_order_current_log', array('delivery_type' => $account['delivery_type']), array('uniacid' => $_W['uniacid'], 'orderid' => $id));
...
if($op == 'cancel') {
	...
	if(!$order['is_pay'] || ($order['is_pay'] == 1 && $order['pay_type'] == 'delivery' || $order['pay_type'] == 'cash')) {
		pdo_update('tiny_wmall1_order', array('status' => 6, 'delivery_status' => 6), array('uniacid' => $_W['uniacid'], 'id' => $id));
...
if($op == 'del') {
	...
	pdo_delete('tiny_wmall1_order', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $id));
	pdo_delete('tiny_wmall1_order_stat', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'oid' => $id));
	pdo_delete('tiny_wmall1_order_comment', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'oid' => $id));
	pdo_delete('tiny_wmall1_order_status_log', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'oid' => $id));
	pdo_delete('tiny_wmall1_order_refund_log', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'oid' => $id));
...
if($op == 'reply_remind') {
	...
	pdo_update('tiny_wmall1_order', array('is_remind' => 0), array('uniacid' => $_W['uniacid'], 'id' => $id));
...
if($op == 'set_deliveryer') {
	...
	$is_store_delivery = pdo_get('tiny_wmall1_store_deliveryer', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'deliveryer_id' => $deliveryer_id));
	...
	foreach($order_ids as $id) {
		$id = intval($id);
		if(!$id) continue;
		pdo_update('tiny_wmall1_order', array('deliveryer_id' => $deliveryer_id, 'delivery_type' => 1,  'status' => '4', 'delivery_status' => '4'), array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $id));
		pdo_update('tiny_wmall1_order_current_log', array('delivery_type' => 1), array('uniacid' => $_W['uniacid'], 'orderid' => $id));
