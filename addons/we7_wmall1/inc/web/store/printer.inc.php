...
global $_W, $_GPC;
$_W['page']['title'] = '打印机管理-' . $_W['we7_wmall1']['config']['title'];
mload()->model('store');
mload()->model('print');
...
if($op == 'list') {
	$data = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall1_printer') . ' WHERE uniacid = :uniacid AND sid = :sid', array(':uniacid' => $_W['uniacid'], ':sid' => $sid));
	if(!empty($data)) {
		foreach($data as &$da) {
...
if($op == 'post') {
	$id = intval($_GPC['id']);
	if($id > 0) {
		$item = pdo_fetch('SELECT * FROM ' . tablename('tiny_wmall1_printer') . ' WHERE uniacid = :uniacid AND id = :id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
	}
	if(!empty($item)) {
	...
	if(checksubmit('submit')) {
		...
		if(!empty($item) && $id) {
			pdo_update('tiny_wmall1_printer', $data, array('uniacid' => $_W['uniacid'], 'id' => $id));
		} else {
			pdo_insert('tiny_wmall1_printer', $data);
		}
...
if($op == 'del') {
	$id = intval($_GPC['id']);
	pdo_delete('tiny_wmall1_printer', array('uniacid' => $_W['uniacid'], 'id' => $id));
...
if($op == 'label_list') {
	if(checksubmit('submit')) {
		if(!empty($_GPC['ids'])) {
			foreach($_GPC['ids'] as $k => $v) {
				...
				pdo_update('tiny_wmall1_printer_label', $data, array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => intval($v)));
			}
	...
	$lists = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall1_printer_label') . $condition . ' ORDER BY displayorder DESC,id ASC', $params);
	include $this->template('store/printer');
}
	...
	if(checksubmit('submit')) {
		if(!empty($_GPC['title'])) {
			foreach($_GPC['title'] as $k => $v) {
				...
				pdo_insert('tiny_wmall1_printer_label', $data);
			}
		}
...
if($op == 'label_del') {
	$id = intval($_GPC['id']);
	pdo_update('tiny_wmall1_goods', array('print_label' => 0), array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'print_label' => $id));
	pdo_delete('tiny_wmall1_printer_label', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $id));
