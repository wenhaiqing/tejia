...
if($op == 'list') {
	...
	$min_id = intval(pdo_fetchcolumn('SELECT min(id) as min_id FROM ' . tablename('tiny_wmall1_order') . $condition , $params));
	$orders = pdo_fetchall('SELECT id, addtime, status, username, mobile, address, location_x, location_y, delivery_status, delivery_type, delivery_time,sid, num, final_fee FROM ' . tablename('tiny_wmall1_order') . $condition . ' order by id desc limit 15', $params, 'id');
	$min = $max = 0;
	if(!empty($orders)) {
		$stores_id = array();
		...
		$stores = pdo_fetchall('select id, title, address, location_x, location_y, telephone from ' . tablename('tiny_wmall1_store') . " where uniacid = :uniacid and id in ({$stores_str})", array(':uniacid' => $_W['uniacid']), 'id');
...
if($op == 'collect') {
	...
	pdo_update('tiny_wmall1_order', array('status' => 4, 'delivery_status' => 7, 'deliveryer_id' => $deliveryer['id'], 'deliveryingtime' => TIMESTAMP), array('uniacid' => $_W['uniacid'], 'id' => $id));
	$note = array(
...
if($op == 'instore') {
	...
	pdo_update('tiny_wmall1_order', array('status' => 4, 'delivery_status' => 4, 'deliveryinstoretime' => TIMESTAMP), array('uniacid' => $_W['uniacid'], 'id' => $id));
	$content = "配送员:{$deliveryer['title']}, 手机号:{$deliveryer['mobile']}";
