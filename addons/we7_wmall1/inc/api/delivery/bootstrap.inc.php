...
if(in_array($do, array('login'))) {
	...
	$deliveryer = pdo_get('tiny_wmall1_deliveryer', array('uniacid' => $_W['uniacid'], 'token' => $token));
	if(empty($deliveryer)) {
		message(ierror(-1, '身份验证失败, 请重新登陆'), '', 'ajax');
	}

	$sids = pdo_fetchall('select sid from ' . tablename('tiny_wmall1_store_deliveryer') . ' where uniacid = :uniacid and deliveryer_id = :deliveryer_id and sid = 0', array(':uniacid' => $_W['uniacid'], ':deliveryer_id' => $deliveryer['id']), 'sid');
	...
	$_W['we7_wmall1']['deliveryer']['type'] = 1;
}
