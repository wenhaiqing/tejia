...
function sms_send($type, $mobile, $content, $sid = 0) {
	global $_W;
	if(!is_array($_W['we7_wmall1']['config']['sms']) || !$_W['we7_wmall1']['config']['sms']['status']) {
		return error(-1, '没有设置短信参数或已关闭短信功能');
	}
	$post = array(
		'method' => 'alibaba.aliqin.fc.sms.num.send',
		'app_key' => $_W['we7_wmall1']['config']['sms']['key'],
		'timestamp' => date('Y-m-d H:i:s'),
		'format' => 'json',
		'v' => '2.0',
		'sign_method' => 'md5',
		'sms_type' => 'normal',
		'sms_free_sign_name' => $_W['we7_wmall1']['config']['sms']['sign'],
		'rec_num' => $mobile,
		'sms_template_code' => $_W['we7_wmall1']['config']['sms'][$type],
	...
	$secret = $_W['we7_wmall1']['config']['sms']['secret'];
...
function sms_singlecall($called_num, $content, $type = 'clerk') {
	global $_W;
	$config = $_W['we7_wmall1']['config']['sms'];
	if(!is_array($_W['we7_wmall1']['config']['sms']) || !$_W['we7_wmall1']['config']['sms']['status']) {
		return error(-1, '没有设置短信参数或已关闭短信功能');
	}
...
function sms_insert_send_log($sid, $type, $mobile) {
	global $_W;
	pdo_query('update ' . tablename('tiny_wmall1_store') . ' set sms_use_times = sms_use_times + 1 where uniacid = :uniacid and id = :id', array(':uniacid' => $_W['uniacid'], ':id' => $sid));
	$data = array(
		'uniacid' => $_W['uniacid'],
		'sid' => $sid,
		'type' => $type,
	...
	pdo_insert('tiny_wmall1_sms_send_log', $data);
	return true;
}
