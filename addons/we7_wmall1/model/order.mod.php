...
function order_fetch($id)
{
	global $_W;
	$id = intval($id);
	$order = pdo_fetch('SELECT * FROM ' . tablename('tiny_wmall1_order') . ' WHERE uniacid = :aid AND id = :id', array(':aid' => $_W['uniacid'], ':id' => $id));
	if (empty($order)) {
		return false;
	}
	...
	if ($order['order_type'] == 3) {
		$table = pdo_get('tiny_wmall1_tables', array('uniacid' => $_W['uniacid'], 'id' => $order['table_id']));
		$order['table'] = $table;
	} elseif ($order['order_type'] == 4) {
		$reserve_type = order_reserve_type();
		$order['reserve_type_cn'] = $reserve_type[$order['reserve_type']]['text'];
		$category = pdo_get('tiny_wmall1_tables_category', array('uniacid' => $_W['uniacid'], 'id' => $order['table_cid']));
...
{
	global $_W;
	$data = pdo_getall('tiny_wmall1_order_discount', array('uniacid' => $_W['uniacid'], 'oid' => $id));
	return $data;
}
function order_place_again($sid, $order_id)
{
	...
	$isexist = pdo_fetchcolumn('SELECT id FROM ' . tablename('tiny_wmall1_order_cart') . " WHERE uniacid = :aid AND sid = :sid AND uid = :uid", array(':aid' => $_W['uniacid'], ':sid' => $sid, ':uid' => $_W['member']['uid']));
	...
	if (empty($isexist)) {
		pdo_insert('tiny_wmall1_order_cart', $data);
	} else {
		pdo_update('tiny_wmall1_order_cart', $data, array('uniacid' => $_W['uniacid'], 'id' => $isexist, 'uid' => $_W['member']['uid']));
	}
...
function order_insert_discount($id, $sid, $discount_data)
{
	...
	if (!empty($discount_data['token'])) {
		pdo_update('tiny_wmall1_activity_coupon_record', array('status' => 2, 'usetime' => TIMESTAMP, 'order_id' => $id), array('uniacid' => $_W['uniacid'], 'id' => $discount_data['token']['recordid']));
	}
	foreach ($discount_data as $data) {
		...
		pdo_insert('tiny_wmall1_order_discount', $insert);
	}
	return true;
}
function order_insert_status_log($id, $sid, $type, $note = '')
{
	...
	pdo_insert('tiny_wmall1_order_status_log', $data);
...
{
	global $_W;
	$data = pdo_fetchall("SELECT * FROM " . tablename('tiny_wmall1_order_status_log') . ' WHERE uniacid = :uniacid and oid = :oid order by id asc', array(':uniacid' => $_W['uniacid'], ':oid' => $id), 'id');
	return $data;
}
function order_fetch_refund_status_log($id)
{
	global $_W;
	$data = pdo_fetchall("SELECT * FROM " . tablename('tiny_wmall1_order_refund_log') . ' WHERE uniacid = :uniacid and oid = :oid and order_type = :order_type order by id asc', array(':uniacid' => $_W['uniacid'], ':oid' => $id, ':order_type' => 'order'), 'id');
	return $data;
}
function order_print($id)
{
	...
	$prints = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall1_printer') . ' WHERE uniacid = :aid AND sid = :sid AND status = 1', array(':aid' => $_W['uniacid'], ':sid' => $sid));
	if (empty($prints)) {
	...
	if ($num > 0) {
		pdo_query('UPDATE ' . tablename('tiny_wmall1_order') . " SET print_nums = print_nums + {$num} WHERE uniacid = {$_W['uniacid']} AND id = {$order['id']}");
	} else {
...
function order_status_notice($sid, $id, $status, $note = '')
{
	...
	if (!empty($order['openid'])) {
		...
		if ($type == 'end') {
			...
			$end_remark = "您的订单已处理完成, 如对商品有不满意或投诉请联系客服:{$_W['we7_wmall1']['config']['mobile']},欢迎您下次光临.戳这里记得给我们的服务评价.";
		}
		if ($type == 'cancel') {
			$title = '订单已取消';
		...
		$url = $_W['siteroot'] . 'app' . ltrim(murl('entry', array('do' => 'order', 'm' => 'we7_wmall1', 'op' => 'detail', 'sid' => $order['sid'], 'id' => $order['id'])), '.');
		$remark = implode("\n", $remark);
		$send = tpl_format($title, $order['ordersn'], $order['status_cn'], $remark);
		$status = $acc->sendTplNotice($order['openid'], $_W['we7_wmall1']['config']['public_tpl'], $send, $url);
		return $status;
	}
	return true;
}
function order_clerk_notice($sid, $id, $type, $note = '')
{
	...
	$url = $_W['siteroot'] . 'app' . ltrim(murl('entry', array('do' => 'mgorder', 'm' => 'we7_wmall1', 'op' => 'detail', 'sid' => $order['sid'], 'id' => $order['id'])), '.');
	$remark = implode("\n", $remark);
	$send = tpl_format($title, $order['ordersn'], $order['status_cn'], $remark);
	mload()->model('sms');
	foreach ($clerks as $clerk) {
		...
		$acc->sendTplNotice($clerk['openid'], $_W['we7_wmall1']['config']['public_tpl'], $send, $url);
	}
	return true;
}
function order_deliveryer_notice($sid, $id, $type, $deliveryer_id = 0, $note = '')
{
	...
	if ($type == 'new_delivery') {
		...
		$remark = implode("\n", $remark);
		$url = $_W['siteroot'] . 'app' . ltrim(murl('entry', array('do' => 'dyorder', 'm' => 'we7_wmall1', 'op' => 'detail', 'id' => $order['id'])), '.');
		...
		if ($type == 'delivery_wait') {
			...
			$remark = implode("\n", $remark);
			$url = $_W['siteroot'] . 'app' . ltrim(murl('entry', array('do' => 'dyorder', 'm' => 'we7_wmall1', 'op' => 'list')), '.');
...
{
	global $_W;
	$data = pdo_get('tiny_wmall1_order_current_log', array('uniacid' => $_W['uniacid'], 'orderid' => $order_id));
	if (!empty($data)) {
		$pay_types = order_pay_types();
...
function order_refund_notice($sid, $order_id, $type, $note = '')
{
	...
	$clerks = clerk_fetchall($sid);
	if ($type == 'apply') {
		$maneger = $_W['we7_wmall1']['config']['manager'];
		if (!empty($maneger['openid'])) {
			...
			$status = $acc->sendTplNotice($maneger['openid'], $_W['we7_wmall1']['config']['notice']['refund_tpl'], $send);
		}
		if (!empty($clerks)) {
			...
			$send = sys_wechat_tpl_format($params);
			foreach ($clerks as $clerk) {
				$status = $acc->sendTplNotice($clerk['openid'], $_W['we7_wmall1']['config']['notice']['refund_tpl'], $send);
			}
		}
	} elseif ($type == 'success') {
		if (!empty($clerks)) {
		...
		if (!empty($current['openid'])) {
			...
			$params = array('first' => $tips, 'reason' => '订单取消, 发起退款流程', 'refund' => $current['fee'], 'remark' => implode("\n", $remark));
			$send = sys_wechat_tpl_format($params);
			$status = $acc->sendTplNotice($current['openid'], $_W['we7_wmall1']['config']['notice']['refund_tpl'], $send);
		}
	}
	return true;
}
...
function order_insert_member_cart($sid)
{
	...
	if (!empty($_GPC['goods'])) {
		...
		$goods_info = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall1_goods') . " WHERE uniacid = :aid AND sid = :sid AND id IN ({$ids_str})", array(':aid' => $_W['uniacid'], ':sid' => $sid), 'id');
		$num_data = array();
		foreach ($_GPC['goods'] as $k => $v) {
			...
			if (!$goods_info[$k]['is_options']) {
				...
				foreach ($v['options'] as $key => $val) {
					...
					$val = intval($val);
					if ($key > 0 && $val > 0) {
						$option = pdo_get('tiny_wmall1_goods_options', array('uniacid' => $_W['uniacid'], 'id' => $key));
						if (empty($option)) {
							continue;
						}
...
{
	global $_W, $_GPC;
	$cart = pdo_fetch('SELECT * FROM ' . tablename('tiny_wmall1_order_cart') . " WHERE uniacid = :aid AND sid = :sid AND uid = :uid", array(':aid' => $_W['uniacid'], ':sid' => $sid, ':uid' => $_W['member']['uid']));
	if (empty($cart)) {
		return false;
	}
	if (TIMESTAMP - $cart['addtime'] > 7 * 86400) {
		pdo_delete('tiny_wmall1_order_cart', array('id' => $cart['id']));
	}
...
{
	global $_W;
	pdo_delete('tiny_wmall1_order_cart', array('sid' => $sid, 'uid' => $_W['member']['uid']));
	return true;
}
function order_update_goods_info($order_id, $sid, $cart = array())
{
	...
	$goods_info = pdo_fetchall('SELECT id,cid,title,price,total,print_label FROM ' . tablename('tiny_wmall1_goods') . " WHERE uniacid = :aid AND sid = :sid AND id IN ({$ids_str})", array(':aid' => $_W['uniacid'], ':sid' => $sid), 'id');
	foreach ($cart['data'] as $k => $v) {
		foreach ($v as $k1 => $v1) {
			pdo_query('UPDATE ' . tablename('tiny_wmall1_goods') . " set sailed = sailed + {$v1['num']} WHERE uniacid = :aid AND id = :id", array(':aid' => $_W['uniacid'], ':id' => $k));
			if (!$k1) {
				if ($goods_info[$k]['total'] != -1 && $goods_info[$k]['total'] > 0) {
					pdo_query('UPDATE ' . tablename('tiny_wmall1_goods') . " set total = total - {$v1['num']} WHERE uniacid = :aid AND id = :id", array(':aid' => $_W['uniacid'], ':id' => $k));
				}
			} else {
				$option = pdo_get('tiny_wmall1_goods_options', array('uniacid' => $_W['uniacid'], 'id' => $k1));
				if (!empty($option) && $option['total'] != -1 && $option['total'] > 0) {
					pdo_query('UPDATE ' . tablename('tiny_wmall1_goods') . " set total = total - {$v1['num']} WHERE uniacid = :aid AND id = :id", array(':aid' => $_W['uniacid'], ':id' => $k1));
				}
			}
			$stat = array();
			if ($k && $v1) {
				...
				pdo_insert('tiny_wmall1_order_stat', $stat);
			}
		}
	}
	pdo_query('UPDATE ' . tablename('tiny_wmall1_store') . " set sailed = sailed + {$cart['num']} WHERE uniacid = :uniacid AND id = :id", array(':uniacid' => $_W['uniacid'], ':id' => $cart['sid']));
	return true;
}
function order_stat_member($sid)
{
	global $_W;
	$is_exist = pdo_get('tiny_wmall1_store_members', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'uid' => $_W['member']['uid']));
	if (empty($is_exist)) {
		...
		pdo_insert('tiny_wmall1_store_members', $insert);
	} else {
		$update = array('last_order_time' => TIMESTAMP);
		pdo_update('tiny_wmall1_store_members', $update, array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'uid' => $_W['member']['uid']));
	}
	return false;
}
function order_amount_stat($sid)
{
	...
	$stat['today_total'] = intval(pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall1_order') . ' where uniacid = :uniacid and sid = :sid and status = 5 and is_pay = 1 and addtime >= :starttime', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':starttime' => $today_starttime)));
	$stat['today_price'] = floatval(pdo_fetchcolumn('select sum(final_fee) from ' . tablename('tiny_wmall1_order') . ' where uniacid = :uniacid and sid = :sid and status = 5 and is_pay = 1 and addtime >= :starttime', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':starttime' => $today_starttime)));
	$stat['month_total'] = intval(pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall1_order') . ' where uniacid = :uniacid and sid = :sid and status = 5 and is_pay = 1 and addtime >= :starttime', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':starttime' => $month_starttime)));
	$stat['month_price'] = floatval(pdo_fetchcolumn('select sum(final_fee) from ' . tablename('tiny_wmall1_order') . ' where uniacid = :uniacid and sid = :sid and status = 5 and is_pay = 1 and addtime >= :starttime', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':starttime' => $month_starttime)));
	return $stat;
}
...
function order_count_activity($sid, $cart, $recordid = 0)
{
	...
	$iscan_use_coupon = 0;
	if ($recordid > 0) {
		$record = pdo_get('tiny_wmall1_activity_coupon_record', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'uid' => $_W['member']['uid'], 'status' => 1, 'id' => $recordid));
		if (!empty($record)) {
			$coupon = pdo_get('tiny_wmall1_activity_coupon', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $record['couponid']));
	...
	if (!empty($activity)) {
		if (!empty($activity['first_order_status'])) {
			$is_first = pdo_get('tiny_wmall1_store_members', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'uid' => $_W['member']['uid']));
			if (empty($is_first)) {
	...
	if ($_GPC['do'] == 'submit') {
		...
		if ($store['delivery_mode'] == 2 && $store['delivery_price'] > 0) {
			if ($_W['member']['setmeal_id'] > 0 && $_W['member']['setmeal_endtime'] >= TIMESTAMP) {
				$nums = pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall1_order') . ' where uniacid = :uniacid and uid = :uid and vip_free_delivery_fee = 1 and status != 6 and addtime >= :addtime', array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid'], ':addtime' => strtotime(date('Y-m-d'))));
...
function order_coupon_available($sid, $price)
{
	...
	$coupons = pdo_fetchall('select a.*,b.title,b.starttime,b.endtime,b.use_limit,b.discount,b.condition from ' . tablename('tiny_wmall1_activity_coupon_record') . ' as a left join ' . tablename('tiny_wmall1_activity_coupon') . ' as b ' . $condition, $params);
	return $coupons;
}
function order_insert_current_log($order_id, $sid, $price, $pay_type, $out_trade_no)
{
	global $_W;
	$log = pdo_get('tiny_wmall1_order_current_log', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'orderid' => $order_id));
	if (!empty($log)) {
		return true;
	}
	$order = pdo_get('tiny_wmall1_order', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $order_id));
	if (empty($order)) {
		return false;
	}
	...
	pdo_insert('tiny_wmall1_order_current_log', $data);
	return true;
}
function order_update_current_pay_type($order_id, $pay_type, $out_trade_no)
{
	global $_W;
	pdo_update('tiny_wmall1_order_current_log', array('is_pay' => 1, 'pay_type' => $pay_type, 'out_trade_no' => $out_trade_no), array('uniacid' => $_W['uniacid'], 'orderid' => $order_id));
	return true;
}
function order_update_current_log($order_id, $status)
{
	global $_W;
	$log = pdo_get('tiny_wmall1_order_current_log', array('uniacid' => $_W['uniacid'], 'orderid' => $order_id));
	if (empty($log)) {
		return false;
	}
	...
	pdo_update('tiny_wmall1_order_current_log', $update, array('uniacid' => $_W['uniacid'], 'orderid' => $order_id));
	if ($status == 5 && $log['trade_status'] != 1) {
		...
		$credit1_config = $_W['we7_wmall1']['config']['credit']['credit1'];
		...
		pdo_update('tiny_wmall1_order_current_log', array('final_fee' => $final_fee), array('uniacid' => $_W['uniacid'], 'orderid' => $order_id));
	}
	return true;
}
function order_build_payrefund($id)
{
	...
	$current = pdo_get('tiny_wmall1_order_current_log', array('uniacid' => $_W['uniacid'], 'orderid' => $order['id']));
	if (empty($current)) {
	...
	pdo_update('tiny_wmall1_order_current_log', array('refund_status' => 1, 'refund_time' => TIMESTAMP, 'out_refund_no' => date('YmdHis') . random(10, true)), array('uniacid' => $_W['uniacid'], 'orderid' => $order['id']));
...
function order_insert_order_refund_log($id, $sid, $type, $note = '')
{
	...
	$notes = array('apply' => array('status' => 1, 'title' => '提交退款申请', 'note' => ""), 'handel' => array('status' => 2, 'title' => "{$_W['we7_wmall1']['config']['title']}接受退款申请", 'note' => ''), 'success' => array('status' => 3, 'title' => "退款成功", 'note' => ''), 'fail' => array('status' => 4, 'title' => "退款失败", 'note' => ''));
	$title = $notes[$type]['title'];
	$note = $note ? $note : $notes[$type]['note'];
	$data = array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'oid' => $id, 'status' => $notes[$type]['status'], 'type' => $type, 'title' => $title, 'note' => $note, 'addtime' => TIMESTAMP);
	pdo_insert('tiny_wmall1_order_refund_log', $data);
	return true;
}
function order_begin_payrefund($id)
{
	global $_W;
	$refund = pdo_get('tiny_wmall1_order_current_log', array('uniacid' => $_W['uniacid'], 'id' => $id));
	if (empty($refund)) {
	...
	if ($refund['pay_type'] == 'credit') {
		if ($refund['uid'] > 0) {
			$log = array($refund['uid'], "外送模块(we7_wmall1)订单退款, 订单号:{$refund['orderid']}, 退款金额:{$refund['fee']}元", 'we7_wmall1');
			load()->model('mc');
			mc_credit_update($refund['uid'], 'credit2', $refund['fee'], $log);
			pdo_update('tiny_wmall1_order_current_log', array('trade_status' => 4, 'refund_status' => 3, 'refund_time' => TIMESTAMP, 'refund_account' => '支付用户的平台余额', 'refund_channel' => 'ORIGINAL'), array('uniacid' => $_W['uniacid'], 'id' => $refund['id']));
		}
		return true;
		...
		pdo_update('tiny_wmall1_order_current_log', array('refund_status' => 2), array('uniacid' => $_W['uniacid'], 'id' => $refund['id']));
		return true;
		...
		pdo_update('tiny_wmall1_order_current_log', array('trade_status' => 4, 'refund_status' => 3, 'refund_time' => TIMESTAMP, 'refund_account' => $response['buyer_logon_id'], 'refund_channel' => 'ORIGINAL'), array('uniacid' => $_W['uniacid'], 'id' => $refund['id']));
		return true;
	}
}
function order_query_payrefund($id)
{
	...
	if ($refund['pay_type'] == 'wechat') {
		...
		if ($response['refund_status_0'] == 'SUCCESS') {
			...
			$update['trade_status'] = 4;
			pdo_update('tiny_wmall1_order_current_log', $update, array('uniacid' => $_W['uniacid'], 'id' => $refund['id']));
		} else {
			pdo_update('tiny_wmall1_order_current_log', $update, array('uniacid' => $_W['uniacid'], 'id' => $refund['id']));
		}
		return true;
