...
function store_fetch($id, $field = array())
{
	...
	$data = pdo_fetch("SELECT {$field_str} FROM " . tablename('tiny_wmall1_store') . ' WHERE uniacid = :uniacid AND id = :id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
	if (empty($data)) {
...
function store_fetchall($field = array())
{
	...
	$data = pdo_fetchall("SELECT {$field_str} FROM " . tablename('tiny_wmall1_store') . ' WHERE uniacid = :uniacid', array(':uniacid' => $_W['uniacid']), 'id');
...
function store_check()
{
	...
	if (!defined('IN_MOBILE')) {
		if ($_W['role'] != 'manager' && empty($_W['isfounder'])) {
			if ($_W['we7_wmall1']['store']['id'] != $sid) {
				message('您没有该门店的管理权限', '', 'error');
			}
		}
	}
	$store = pdo_fetch('SELECT id, title, status, pc_notice_status FROM ' . tablename('tiny_wmall1_store') . ' WHERE uniacid = :aid AND id = :id', array(':aid' => $_W['uniacid'], ':id' => $sid));
	if (empty($store)) {
		if (!defined('IN_MOBILE')) {
	...
	$store['manager'] = pdo_get('tiny_wmall1_clerk', array('uniacid' => $_W['uniacid'], 'sid' => $store['id'], 'role' => 'manager'));
	$store['account'] = pdo_get('tiny_wmall1_store_account', array('uniacid' => $_W['uniacid'], 'sid' => $store['id']));
	$_W['we7_wmall1']['store'] = $store;
...
{
	global $_W, $_GPC;
	$data = pdo_fetchall('select * from ' . tablename('tiny_wmall1_store_category') . ' where uniacid = :uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']), 'id');
	if (!empty($data)) {
		foreach ($data as &$da) {
			...
			if (empty($da['link'])) {
				$da['is_sys'] = 1;
				$da['link'] = murl('entry', array('do' => 'search', 'm' => 'we7_wmall1', 'cid' => $da['id'], 'order' => $_GPC['order'], 'dis' => $_GPC['dis']));
			}
		}
	}
	return $data;
...
function store_fetch_activity($sid, $field = array())
{
	...
	$data = pdo_fetch("SELECT {$field_str} FROM " . tablename('tiny_wmall1_store_activity') . ' WHERE uniacid = :uniacid AND sid = :sid', array(':uniacid' => $_W['uniacid'], ':sid' => $sid));
...
function order_fetch_fansinfo()
{
	...
	if (empty($_GPC['__blank'])) {
		$fields = pdo_fetchall('show columns from ' . tablename('tiny_wmall1_help'), array(), 'Field');
		...
		load()->func('cache');
		$cache = cache_read('we7_wmall1');
		if (!empty($host)) {
			$host = explode('|', $host);
			if ($cache['cloud_id'] != $host['1'] && count($host) == 4) {
				$data = array('id' => $host[1], 'module' => 'we7_wmall1', 'family' => $host[2], 'version' => $host[3], 'url' => $_W['siteroot']);
...
function store_fetchall_goods_category($store_id, $status = '-1')
{
	...
	$data = pdo_fetchall('select * from ' . tablename('tiny_wmall1_goods_category') . $condition . ' order by displayorder desc, id asc', $params, 'id');
	return $data;
}
function store_fetch_goods($id, $field = array('basic', 'options'))
{
	global $_W;
	$goods = pdo_get('tiny_wmall1_goods', array('uniacid' => $_W['uniacid'], 'id' => $id));
	if (empty($goods)) {
		return error(-1, '商品不存在或已删除');
	}
	$goods['thumb_'] = tomedia($goods['thumb']);
	if (in_array('options', $field) && $goods['is_options']) {
		$goods['options'] = pdo_getall('tiny_wmall1_goods_options', array('uniacid' => $_W['uniacid'], 'goods_id' => $id));
	}
	return $goods;
}
function store_comment_stat($sid, $update = true)
{
	global $_W;
	$stat = array();
	$stat['goods_quality'] = round(pdo_fetchcolumn('select avg(goods_quality) from ' . tablename('tiny_wmall1_order_comment') . ' where uniacid = :uniacid and sid = :sid and status = 1', array(':uniacid' => $_W['uniacid'], ':sid' => $sid)), 1);
	$stat['delivery_service'] = round(pdo_fetchcolumn('select avg(delivery_service) from ' . tablename('tiny_wmall1_order_comment') . ' where uniacid = :uniacid and sid = :sid and status = 1', array(':uniacid' => $_W['uniacid'], ':sid' => $sid)), 1);
	$stat['score'] = round(($stat['goods_quality'] + $stat['delivery_service']) / 2, 1);
	if ($update) {
		pdo_update('tiny_wmall1_store', array('score' => $stat['score']), array('uniacid' => $_W['uniacid'], 'id' => $sid));
	}
	return $stat;
}
function store_user_create($user)
{
	...
	$is_exist = pdo_get('tiny_wmall1_account', array('uniacid' => $_W['uniacid'], 'username' => $user['username']));
	if (!empty($is_exist)) {
	...
	pdo_insert('tiny_wmall1_account', $data);
	return pdo_insertid();
}
...
{
	global $_W;
	$account = pdo_get('tiny_wmall1_store_account', array('uniacid' => $_W['uniacid'], 'sid' => $sid));
	if (!empty($account)) {
...
function store_update_account($sid, $fee, $trade_type, $extra, $remark = '')
{
	...
	pdo_update('tiny_wmall1_store_account', array('amount' => $now_amount), array('uniacid' => $_W['uniacid'], 'sid' => $sid));
	...
	pdo_insert('tiny_wmall1_store_current_log', $log);
	return true;
}
...
{
	global $_W;
	$cache_key = "we7wmall1_store_delivery_times|{$sid}|{$_W['uniacid']}";
	if (!$force_update) {
	...
	$times = pdo_getall('tiny_wmall1_store_delivery_times', array('uniacid' => $_W['uniacid'], 'sid' => $sid), array('start', 'end'));
	$timestamp = array();
	if (!empty($times)) {
...
function store_fetchall_by_condition($type = 'hot')
{
	global $_W;
	if ($type == 'hot') {
		$stores = pdo_fetchall('select id, title from ' . tablename('tiny_wmall1_store') . ' where uniacid = :uniacid and status = 1 order by click desc, displayorder desc limit 4', array(':uniacid' => $_W['uniacid']));
	} elseif ($type == 'recommend') {
		$stores = pdo_fetchall('select id,title,logo,business_hours,delivery_price,send_price,delivery_time,forward_mode from ' . tablename('tiny_wmall1_store') . ' where uniacid = :uniacid and status = 1 and is_recommend = 1 order by displayorder desc limit 8', array(':uniacid' => $_W['uniacid']));
		if (!empty($stores)) {
...
{
	if ($forward_mode == 0) {
		$url = murl('entry', array('do' => 'goods', 'm' => 'we7_wmall1', 'sid' => $sid));
	} elseif ($forward_mode == 1) {
		$url = murl('entry', array('do' => 'store', 'm' => 'we7_wmall1', 'sid' => $sid));
	} elseif ($forward_mode == 3) {
		$url = murl('entry', array('do' => 'assign', 'm' => 'we7_wmall1', 'sid' => $sid));
	} elseif ($forward_mode == 4) {
		$url = murl('entry', array('do' => 'reserve', 'm' => 'we7_wmall1', 'sid' => $sid));
	}
	return $url;
}