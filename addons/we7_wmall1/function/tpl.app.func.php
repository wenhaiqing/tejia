...
function tpl_image($name, $value) {
	$url = empty($value) ? '../addons/we7_wmall1/resource/app/img/add_pic.png' : tomedia($value);
	if (!defined('TPL_INIT_TINY_IMAGE')) {
		$html = '
		<script>
			function uploadImage(obj){
				$.getScript("../addons/we7_wmall1/resource/app/js/tiny.js", function(){
...
function tpl_mutil_image($name, $values, $file_nums = 9) {
	...
	$html .= '
		<div class="col-25 image-item image-add" onclick="uploadMutilImage(this)">
			<img src="../addons/we7_wmall1/resource/app/img/add_pic.png" alt=""/>
		</div>
	';
	$html .= '</div>';
	return $html;
