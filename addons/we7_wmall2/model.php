...
class Mloader
{
	...
	function func($name)
	{
		...
		$file = IA_ROOT . '/addons/we7_wmall2/function/' . $name . '.func.php';
		if (file_exists($file)) {
			include $file;
			$this->cache['func'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid Helper Function /addons/we7_wmall2/function/' . $name . '.func.php', E_USER_ERROR);
			return false;
		}
	}
	function model($name)
	{
		...
		$file = IA_ROOT . '/addons/we7_wmall2/model/' . $name . '.mod.php';
		if (file_exists($file)) {
			include $file;
			$this->cache['model'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid Model /addons/we7_wmall2/model/' . $name . '.mod.php', E_USER_ERROR);
			return false;
		}
	}
	function classs($name)
	{
		...
		$file = IA_ROOT . '/addons/we7_wmall2/class/' . $name . '.class.php';
		if (file_exists($file)) {
			include $file;
			$this->cache['class'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid Class /addons/we7_wmall2/class/' . $name . '.class.php', E_USER_ERROR);
			return false;
		}
	}
}
...
function sys_config($uniacid = 0)
{
	...
	$data = pdo_fetch("SELECT * FROM " . tablename('tiny_wmall2_config') . ' WHERE uniacid = :uniacid', array(':uniacid' => $uniacid));
	if (empty($data)) {
...
function sys_settle_config($uniacid = 0)
{
	...
	$data = pdo_fetch("SELECT * FROM " . tablename('tiny_wmall2_store_settle_config') . ' WHERE uniacid = :uniacid', array(':uniacid' => $uniacid));
	if (empty($data)) {
...
{
	global $_W;
	$slides = pdo_getall('tiny_wmall2_slide', array('uniacid' => $_W['uniacid'], 'type' => $type, 'status' => 1));
	if ($type == 1) {
		shuffle($slides);
	}
	return $slides;
...
function sys_store_cron()
{
	...
	if ($store['pay_time_limit'] > 0) {
		$orders = pdo_fetchall('select id, addtime from ' . tablename('tiny_wmall2_order') . ' where uniacid = :uniacid and sid = :sid and is_pay = 0 and status = 1 and order_type <= 2 and addtime <= :addtime limit 5', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':addtime' => time() - $store['pay_time_limit'] * 60));
		if (!empty($orders)) {
			foreach ($orders as $order) {
				pdo_update('tiny_wmall2_order', array('status' => 6, 'delivery_status' => 6), array('sid' => $sid, 'id' => $order['id']));
	...
	if ($store['auto_end_hours'] > 0) {
		$orders = pdo_fetchall('select id, sid, deliveryingtime from ' . tablename('tiny_wmall2_order') . ' where uniacid = :uniacid and sid = :sid and status = 4 and order_type <= 2 and deliveryingtime > 0 and deliveryingtime <= :deliveryingtime order by id asc limit 5', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':deliveryingtime' => time() - $store['auto_end_hours'] * 3600));
		if (!empty($orders)) {
			foreach ($orders as $order) {
				pdo_update('tiny_wmall2_order', array('status' => 5, 'delivery_status' => 5, 'deliveryedtime' => TIMESTAMP), array('sid' => $sid, 'id' => $order['id']));
...
function operator_menu()
{
	...
	if ($sid > 0) {
		$store = pdo_get('tiny_wmall2_store', array('uniacid' => $_W['uniacid'], 'id' => $sid), array('id', 'title'));
		$menu[] = array('title' => "当前门店:{$store['title']}", 'items' => array(array('title' => '门店信息', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'store', 'op' => 'post', 'id' => $sid))), array('title' => '订单管理', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'order'))), array('title' => '配货中心', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'dispatch'))), array('title' => '商品分类', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'category'))), array('title' => '商品列表', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'goods'))), array('title' => '打印机管理', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'printer'))), array('title' => '店员管理', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'clerk'))), array('title' => '配送员管理', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'deliveryer'))), array('title' => '评价管理', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'comment'))), array('title' => '顾客管理', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'member', 'op' => 'list'))), array('title' => '营销活动', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'activity'))), array('title' => '订单统计', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'stat'))), array('title' => '顾客统计', 'url' => url('site/entry', array('m' => 'we7_wmall2', 'do' => 'member', 'op' => 'stat')))));
	}
	return $menu;
}
...
function sys_notice_settle($sid, $type = 'clerk', $note = '')
{
	...
	$store['manager'] = pdo_get('tiny_wmall2_clerk', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'role' => 'manager'));
	...
	if ($type == 'clerk') {
		...
		$tips = "【{$store['title']}】申请入驻【{$_W['we7_wmall2']['config']['title']}】进度通知";
		...
		$status = $acc->sendTplNotice($store['manager']['openid'], $_W['we7_wmall2']['config']['notice']['settle_tpl'], $send);
	} elseif ($type == 'manager') {
		...
		$status = $acc->sendTplNotice($maneger['openid'], $_W['we7_wmall2']['config']['notice']['settle_apply_tpl'], $send);
	}
	return $status;
}
...
function sys_notice_store_getcash($sid, $getcash_log_id, $type = 'apply', $note = '')
{
	...
	$store['manager'] = pdo_get('tiny_wmall2_clerk', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'role' => 'manager'));
	$log = pdo_get('tiny_wmall2_store_getcash_log', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $getcash_log_id));
	if (empty($log)) {
	...
	if ($type == 'apply') {
		if (!empty($store['manager']) && !empty($store['manager']['openid'])) {
			...
			$status = $acc->sendTplNotice($store['manager']['openid'], $_W['we7_wmall2']['config']['notice']['getcash_apply_tpl'], $send);
		}
		$maneger = $_W['we7_wmall2']['config']['manager'];
		if (!empty($maneger['openid'])) {
			...
			$params = array('first' => $tips, 'money' => $log['final_fee'], 'timet' => date('Y-m-d H:i', TIMESTAMP), 'remark' => implode("\n", $remark));
			$send = sys_wechat_tpl_format($params);
			$status = $acc->sendTplNotice($maneger['openid'], $_W['we7_wmall2']['config']['notice']['getcash_apply_tpl'], $send);
		}
	} elseif ($type == 'success') {
		if (empty($store['manager']) || empty($store['manager']['openid'])) {
		...
		$send = sys_wechat_tpl_format($params);
		$status = $acc->sendTplNotice($store['manager']['openid'], $_W['we7_wmall2']['config']['notice']['getcash_success_tpl'], $send);
	} elseif ($type == 'fail') {
		if (empty($store['manager']) || empty($store['manager']['openid'])) {
			return error(-1, '门店管理员信息不完善');
		}
		...
		$send = sys_wechat_tpl_format($params);
		$status = $acc->sendTplNotice($store['manager']['openid'], $_W['we7_wmall2']['config']['notice']['getcash_fail_tpl'], $send);
	}
	return $status;
}
...
function sys_delivery_config($uniacid = 0)
{
	...
	$config = pdo_get('tiny_wmall2_delivery_config', array('uniacid' => $uniacid));
	if (empty($config)) {
...
{
	global $_W;
	$deliveryer = pdo_get('tiny_wmall2_deliveryer', array('uniacid' => $_W['uniacid'], 'id' => $deliveryer_id));
	if (empty($deliveryer)) {
		return error(-1, '配送员不存在');
	}
	$log = pdo_get('tiny_wmall2_deliveryer_getcash_log', array('uniacid' => $_W['uniacid'], 'deliveryer_id' => $deliveryer_id, 'id' => $getcash_log_id));
	if (empty($log)) {
		return error(-1, '提现记录不存在');
	}
	$acc = WeAccount::create($_W['acid']);
	if ($type == 'apply') {
		if (!empty($deliveryer['openid'])) {
			...
			$send = sys_wechat_tpl_format($params);
			$status = $acc->sendTplNotice($deliveryer['openid'], $_W['we7_wmall2']['config']['notice']['getcash_apply_tpl'], $send);
		}
		$maneger = $_W['we7_wmall2']['config']['manager'];
		if (!empty($maneger['openid'])) {
		...
		$send = sys_wechat_tpl_format($params);
		$status = $acc->sendTplNotice($deliveryer['openid'], $_W['we7_wmall2']['config']['notice']['getcash_success_tpl'], $send);
	} elseif ($type == 'fail') {
		if (empty($deliveryer['openid'])) {
			return error(-1, '配送员信息不完善');
		}
		...
		$send = sys_wechat_tpl_format($params);
		$status = $acc->sendTplNotice($deliveryer['openid'], $_W['we7_wmall2']['config']['notice']['getcash_fail_tpl'], $send);
	}
	return $status;
}
