...
if($op == 'index') {
	...
	$data = pdo_getall('tiny_wmall2_reserve', array('uniacid' => $_W['uniacid'], 'sid' => $sid));
	if(!empty($data)) {
		$reserves = array();
		foreach($data as $da) {
...
if($op == 'post') {
	...
	$time = trim($_GPC['time']);
	$cid = intval($_GPC['cid']);
	$category = pdo_get('tiny_wmall2_tables_category', array('uniacid' => $_W['uniacid'], 'id' => $cid, 'sid' => $sid));
	if(empty($category)) {
...
if($op == 'submit') {
	...
	pdo_insert('tiny_wmall2_order', $order);
	$id = pdo_insertid();
