...
if($op == 'store') {
	...
	$store = pdo_get('tiny_wmall2_store', array('uniacid' => $_W['uniacid'], 'id' => $account['sid']));
	if(!empty($store)) {
		$mine_url = $this->createMobileUrl('mine');
		$admin_url = $_W['siteroot'].'addons/we7_wmall2/admin';
		if($store['status'] <= 1) {
	...
	if($_W['isajax']) {
		...
		pdo_update('tiny_wmall2_clerk', array('sid' => $store_id), array('uniacid' => $_W['uniacid'], 'id' => $account['id']));
