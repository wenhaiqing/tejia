...
if($op == 'index') {
	$categorys = pdo_fetchall('select * from ' . tablename('tiny_wmall2_errander_category') . ' where uniacid = :uniacid and status = 1 order by displayorder desc', array(':uniacid' => $_W['uniacid']));
	...
	$orders = pdo_fetchall('select a.*,b.title,b.thumb from ' . tablename('tiny_wmall2_errander_order') . ' as a left join ' . tablename('tiny_wmall2_errander_category') . ' as b on a.order_cid = b.id where a.uniacid = :uniacid order by a.id desc limit 5', array(':uniacid' => $_W['uniacid']));
	$delivery_num = pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall2_store_deliveryer') . ' where uniacid = :uniacid and sid = 0', array(':uniacid' => $_W['uniacid']));
...
if($op == 'post') {
	...
	pdo_insert('tiny_wmall2_errander_order', $order);
	$id = pdo_insertid();
