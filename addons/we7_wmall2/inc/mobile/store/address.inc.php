...
if($op == 'post') {
	...
	if($_W['ispost']) {
		...
		if(!empty($address['id'])) {
			pdo_update('tiny_wmall2_address', $data, array('uniacid' => $_W['uniacid'], 'id' => $id));
		} else {
			pdo_insert('tiny_wmall2_address', $data);
			$id = pdo_insertid();
		}
...
if($op == 'del') {
	...
	$id = intval($_GPC['id']);
	pdo_delete('tiny_wmall2_address', array('uniacid' => $_W['uniacid'], 'id' => $id));
	message(error(0, ''), '', 'ajax');
}

if($op == 'default') {
	$id = intval($_GPC['id']);
	pdo_update('tiny_wmall2_address', array('is_default' => 0), array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid'], 'type' => 1));
	pdo_update('tiny_wmall2_address', array('is_default' => 1), array('uniacid' => $_W['uniacid'], 'id' => $id));
	exit();
}
