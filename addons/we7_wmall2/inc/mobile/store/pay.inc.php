...
$tables = array(
	'order' => 'tiny_wmall2_order',
	'card' => 'tiny_wmall2_delivery_cards_order',
	'errander' => 'tiny_wmall2_errander_order'
);
...
$record = pdo_get('tiny_wmall2_paylog', array('uniacid' => $_W['uniacid'], 'order_id' => $id, 'order_type' => $type));
if(empty($record)) {
	$record = array(
		'uniacid' => $_W['uniacid'],
	...
	pdo_insert('tiny_wmall2_paylog', $record);
} else {
	if($record['status'] == 1) {
...
$available_pay_types = $_W['we7_wmall2']['config']['payment'];
