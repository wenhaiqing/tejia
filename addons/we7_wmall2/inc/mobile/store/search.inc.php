...
global $_W, $_GPC;
$do = 'search';
$title = "{$_W['we7_wmall2']['config']['title']}-商家列表";
...
if($op == 'list') {
	...
	$stores = pdo_fetchall('select id,title,logo,sailed,score,business_hours,is_in_business,delivery_price,delivery_free_price,send_price,delivery_time,delivery_mode,token_status,invoice_status,location_x,location_y,forward_mode from ' . tablename('tiny_wmall2_store') . $condition, $params);
	$min = 0;
	if(!empty($stores)) {
		foreach($stores as &$row) {
			...
			$row['hot_goods'] = pdo_fetchall('select title from ' . tablename('tiny_wmall2_goods') . ' where uniacid = :uniacid and sid = :sid and is_hot = 1 limit 3', array(':uniacid' => $_W['uniacid'], ':sid' => $row['id']));
