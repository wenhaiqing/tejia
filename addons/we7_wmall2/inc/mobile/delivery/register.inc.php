...
$delivery = pdo_get('tiny_wmall2_deliveryer', array('uniacid' => $_W['uniacid'], 'openid' => $fans['openid']));
if(!empty($delivery)) {
...
if($_W['isajax']) {
	...
	$is_exist = pdo_fetchcolumn('select id from ' . tablename('tiny_wmall2_deliveryer') . ' where uniacid = :uniacid and mobile = :mobile', array(':uniacid' => $_W['uniacid'], ':mobile' => $user['mobile']));
	if(!empty($is_exist)) {
		message(error(-1, '该手机号已绑定其他配送员, 请更换手机号'), '', 'ajax');
	}
	$is_exist = pdo_fetchcolumn('select id from ' . tablename('tiny_wmall2_deliveryer') . ' where uniacid = :uniacid and openid = :openid', array(':uniacid' => $_W['uniacid'], ':openid' => $user['openid']));
	if(!empty($is_exist)) {
		message(error(-1, '该微信号已绑定其他配送员, 请更换微信号'), '', 'ajax');
	}
	$user['salt'] = random(6);
	$user['password'] = md5(md5($user['salt'] . $user['password']) . $user['salt']);
	pdo_insert('tiny_wmall2_deliveryer', $user);
	deliveryer_all(true);
