...
$delivery_type = $_W['we7_wmall2']['deliveryer']['type'];
$delivery_stores = implode(', ', $_W['we7_wmall2']['deliveryer']['store']);
$deliveryer = $_W['we7_wmall2']['deliveryer']['user'];
...
if($op == 'list') {
	...
	$orders = pdo_fetchall('SELECT id, addtime, status, username, mobile, address, delivery_status, delivery_type, delivery_time,sid, num, final_fee FROM ' . tablename('tiny_wmall2_order') . $condition, $params, 'id');
	$min = 0;
	if(!empty($orders)) {
		$stores_id = array();
		...
		$stores = pdo_fetchall('select id, title, address from ' . tablename('tiny_wmall2_store') . " where uniacid = :uniacid and id in ({$stores_str})", array(':uniacid' => $_W['uniacid']), 'id');
...
if($op == 'more') {
	...
	$orders = pdo_fetchall('select * from ' . tablename('tiny_wmall2_order') . ' where uniacid = :uniacid and delivery_status = :delivery_status and deliveryer_id = :deliveryer_id and id < :id order by id desc limit 15', array(':uniacid' => $_W['uniacid'], ':delivery_status' => $status, ':deliveryer_id' => $deliveryer['id'], ':id' => $id), 'id');
	$min = 0;
	if(!empty($orders)) {
		$delivery_status = order_delivery_status();
		foreach ($orders as &$row) {
			...
			$row['store_address'] = pdo_fetchcolumn('select address from ' . tablename('tiny_wmall2_store') . ' where uniacid = :uniacid and id = :sid', array(':uniacid' => $_W['uniacid'], ':sid' => $row['sid']));
		}
		$min = min(array_keys($orders));
	}
...
if($op == 'collect') {
	...
	pdo_update('tiny_wmall2_order', array('status' => 4, 'delivery_status' => 4, 'deliveryer_id' => $deliveryer['id'], 'deliveryingtime' => TIMESTAMP), array('uniacid' => $_W['uniacid'], 'id' => $id));
	$note = array(
	...
	if($order['delivery_type'] == 2) {
		...
		pdo_update('tiny_wmall2_order_current_log', array('deliveryer_fee' => $order['deliveryer_fee'], 'deliveryer_id' => $deliveryer['id'], 'store_deliveryer_fee' => $order['store_deliveryer_fee'], 'vip_free_delivery_fee' => $order['vip_free_delivery_fee'],  'order_status' => 4), array('uniacid' => $_W['uniacid'], 'orderid' => $order['id']));
