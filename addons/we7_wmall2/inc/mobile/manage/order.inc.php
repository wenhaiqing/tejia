...
if($op == 'list') {
	...
	$orders = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_order') . $condition . ' order by id desc limit 5', $params, 'id');
	$min = 0;
	if(!empty($orders)) {
...
if($op == 'more') {
	...
	$order_status = order_status();
	$id = intval($_GPC['id']);
	$orders = pdo_fetchall('select * from ' . tablename('tiny_wmall2_order') . ' where uniacid = :uniacid and sid = :sid and id < :id order by id desc limit 5', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':id' => $id), 'id');
	$min = 0;
	if(!empty($orders)) {
...
if($op == 'detail') {
	...
	$log = pdo_fetch('select * from ' . tablename('tiny_wmall2_order_status_log') . ' where uniacid = :uniacid and oid = :oid order by id desc', array(':uniacid' => $_W['uniacid'], ':oid' => $id));
...
if($op == 'status') {
	$id = $_GPC['id'];
	$order = pdo_get('tiny_wmall2_order', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $id));
	if(empty($order)) {
...
if($op == 'deliveryer') {
	...
	$id = intval($_GPC['id']);
	pdo_update('tiny_wmall2_order', array('deliveryer_id' => $deliveryer_id, 'delivery_type' => 1, 'status' => '4', 'delivery_status' => '4', 'deliveryingtime' => TIMESTAMP), array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $id));
...
if($op == 'consume') {
	$id = intval($_GPC['id']);
	$order = pdo_get('tiny_wmall2_order', array('uniacid' => $_W['uniacid'], 'id' => $id));
	if(empty($order)) {
	...
	pdo_update('tiny_wmall2_order', array('status' => 5, 'delivery_status' => 5, 'deliveryedtime' => TIMESTAMP), array('uniacid' => $_W['uniacid'], 'id' => $id));
