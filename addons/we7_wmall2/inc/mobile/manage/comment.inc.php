...
if($op == 'list') {
	...
	$comments = pdo_fetchall('select * from ' . tablename('tiny_wmall2_order_comment') . $condition . ' order by id desc limit 15', $params, 'id');
	$min = 0;
	if(!empty($comments)) {
...
if($op == 'status') {
	$id = intval($_GPC['id']);
	$comment = pdo_get('tiny_wmall2_order_comment', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $id));
	if(empty($comment)) {
	...
	pdo_update('tiny_wmall2_order_comment', array('status' => $status), array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $id));
...
if($op == 'reply') {
	...
	pdo_update('tiny_wmall2_order_comment', array('reply' => $content, 'status' => 1, 'replytime' => TIMESTAMP, 'status' => 1), array('uniacid' => $_W['uniacid'], 'id' => $id));
	store_comment_stat($comment['sid']);
	message(error(0, ''), '', 'ajax');
}
