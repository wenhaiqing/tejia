...
global $_W, $_GPC;
$_W['page']['title'] = '财务统计-' . $_W['we7_wmall2']['config']['title'];
mload()->model('finance');
$op = trim($_GPC['op']) ? trim($_GPC['op']) : 'account';

if($op == 'account') {
	$stores = pdo_getall('tiny_wmall2_store', array('uniacid' => $_W['uniacid']), array('id', 'title', 'logo'), 'id');
	if(!empty($stores)) {
		$stores_ids = implode(',', array_keys($stores));
		pdo_query('delete from ' . tablename('tiny_wmall2_store_account') . " where uniacid = :uniacid and sid not in ({$stores_ids})", array(':uniacid' => $_W['uniacid']));
	}
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall2_store_account') .  $condition, $params);
	$accounts = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_store_account') . $condition . ' ORDER BY id ASC LIMIT '.($pindex - 1) * $psize.','.$psize, $params);
...
if($op == 'set') {
	...
	if(checksubmit()) {
		...
		if(empty($config)) {
			$data['amount'] = 0.00;
			pdo_insert('tiny_wmall2_store_account', $data);
		} else {
			pdo_update('tiny_wmall2_store_account', $data, array('uniacid' => $_W['uniacid'], 'sid' => $sid));
			$update = array(
			...
			pdo_update('tiny_wmall2_store', $update, array('uniacid' => $_W['uniacid'], 'id' => $sid));
		}
		pdo_query('update ' .  tablename('tiny_wmall2_store_deliveryer') . ' set delivery_type = :delivery_type where uniacid = :uniacid and sid = :sid', array(':uniacid' => $_W['uniacid'], ':delivery_type' => $data['delivery_type'], ':sid' => $sid));
...
if($op == 'currentlog') {
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall2_order_current_log') .  $condition, $params);
	$records = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_order_current_log') . $condition . ' ORDER BY id DESC LIMIT '.($pindex - 1) * $psize.','.$psize, $params);
	$pager = pagination($total, $pindex, $psize);
...
if($op == 'getcashlog') {
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall2_store_getcash_log') .  $condition, $params);
	$records = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_store_getcash_log') . $condition . ' ORDER BY id DESC LIMIT '.($pindex - 1) * $psize.','.$psize, $params);
	if(!empty($records)) {
...
if($op == 'gatcashstatus') {
	$id = intval($_GPC['id']);
	$status = intval($_GPC['status']);
	pdo_update('tiny_wmall2_store_getcash_log', array('status' => $status, 'endtime' => TIMESTAMP), array('uniacid' => $_W['uniacid'], 'id' => $id));
	message('设置提现状态成功', referer(), 'success');
}

if($op == 'transfers') {
	$id = intval($_GPC['id']);
	$log = pdo_get('tiny_wmall2_store_getcash_log', array('uniacid' => $_W['uniacid'], 'id' => $id));
	if(empty($log)) {
	...
	pdo_update('tiny_wmall2_store_getcash_log', array('status' => 1, 'endtime' => TIMESTAMP), array('uniacid' => $_W['uniacid'], 'id' => $id));
	message('打款成功', referer(), 'success');
}

if($op == 'begin_refund') {
	$id = intval($_GPC['id']);
	$record = pdo_get('tiny_wmall2_order_current_log', array('uniacid' => $_W['uniacid'], 'id' => $id));
	if(empty($record)) {
...
if($op == 'query_refund') {
	...
	if(is_error($query)) {
		...
		$current = pdo_get('tiny_wmall2_order_current_log', array('uniacid' => $_W['uniacid'], 'id' => $id));
		if($current['refund_status'] == 3) {
			order_insert_order_refund_log($current['orderid'], $current['sid'], 'success');
			order_refund_notice($current['sid'], $current['orderid'], 'success');
		}
