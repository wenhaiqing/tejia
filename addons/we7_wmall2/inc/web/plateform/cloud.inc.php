...
$_W['page']['title'] = "云服务-{$_W['we7_wmall2']['config']['title']}";
...
if($op == 'auth') {
	...
	if(checksubmit()) {
		...
		$status = cloud_w_query_auth($code, 'we7_wmall2');
		if(is_error($status)) {
...
if($op == 'upgrade')  {
	...
	cache_load('we7_wmall2_upgrade');
	if(!empty($_W['cache']['we7_wmall2_upgrade'])) {
		$upgrade = $_W['cache']['we7_wmall2_upgrade'];
	}
...
if($op == 'process') {
	...
	if(!empty($packet) && !empty($packet['upgrade'])) {
		...
		cache_delete('checkupgrade:we7_wmall2');
		message('更新已完成. ', $this->createWebUrl('ptfcloud', array('op' => 'upgrade')), 'info');
	}
}
if($op == 'check') {
	$lock = cache_load('checkupgrade:we7_wmall2');
	if(empty($lock) || (TIMESTAMP - 1800 > $lock['lastupdate'])) {
		...
		if(!is_error($upgrade) && !empty($upgrade['upgrade'])) {
			...
			cache_write('checkupgrade:we7_wmall2', $upgrade);
