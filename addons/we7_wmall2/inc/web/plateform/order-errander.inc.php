...
global $_W, $_GPC;
$_W['page']['title'] = '跑腿订单-' . $_W['we7_wmall2']['config']['title'];
...
if($op == 'list') {
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall2_errander_order') .  $condition, $params);
	$data = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_errander_order') . $condition . ' ORDER BY id DESC LIMIT '.($pindex - 1) * $psize.','.$psize, $params, 'id');
...
if($op == 'del') {
	...
	foreach($ids as $id) {
		$id = intval($id);
		$order = pdo_get('tiny_wmall2_errander_order', array('uniacid' => $_W['uniacid'], 'id' => $id));
		if($order['status'] != 4) {
			message('订单状态有误， 不能删除订单', referer(), 'error');
		}
		pdo_delete('tiny_wmall2_errander_order', array('uniacid' => $_W['uniacid'], 'id' => $id));
		pdo_delete('tiny_wmall2_errander_order_status_log', array('uniacid' => $_W['uniacid'], 'oid' => $id));
		pdo_delete('tiny_wmall2_order_refund_log', array('uniacid' => $_W['uniacid'], 'oid' => $id, 'order_type' => 'errander'));
	}
