...
if($op == 'set') {
	...
	if(checksubmit()) {
		...
		pdo_update('tiny_wmall2_config', array('errander' => iserializer($errander)), array('uniacid' => $_W['uniacid']));
...
if($op == 'category_post') {
	$id = intval($_GPC['id']);
	if($id > 0) {
		$item = pdo_get('tiny_wmall2_errander_category', array('uniacid' => $_W['uniacid'], 'id' => $id));
		if(empty($item)) {
	...
	if(checksubmit()) {
		...
		if(empty($item['id'])) {
			pdo_insert('tiny_wmall2_errander_category', $data);
		} else {
			pdo_update('tiny_wmall2_errander_category', $data, array('uniacid' => $_W['uniacid'], 'id' => $id));
		}
...
if($op == 'category_list') {
	if(checksubmit('submit')) {
		if(!empty($_GPC['ids'])) {
			foreach($_GPC['ids'] as $k => $v) {
				...
				pdo_update('tiny_wmall2_errander_category', $data, array('uniacid' => $_W['uniacid'], 'id' => intval($v)));
			}
			message('编辑成功', $this->createWebUrl('ptfconfig-errander', array('op' => 'category_list')), 'success');
		}
	}
	...
	$categorys = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_errander_category') . $condition . ' ORDER BY displayorder DESC,id ASC', $params);
	include $this->template('plateform/config-errander');
}

if($op == 'category_del') {
	$id = intval($_GPC['id']);
	pdo_delete('tiny_wmall2_errander_category', array('uniacid' => $_W['uniacid'], 'id' => $id));
...
if($op == 'category_status') {
	...
	pdo_update('tiny_wmall2_errander_category', array('status' => $status), array('uniacid' => $_W['uniacid'], 'id' => $id));
	exit();
}