...
global $_W, $_GPC;
$_W['page']['title'] = '外卖订单-' . $_W['we7_wmall2']['config']['title'];
...
if($op == 'list') {
	...
	$wait_total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall2_order') . ' WHERE uniacid = :uniacid AND sid = :sid and status = 1', array(':uniacid' => $_W['uniacid'], ':sid' => $sid));
...
if($op == 'export') {
	...
	$list = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_order') . $condition . ' ORDER BY id DESC', $params);
	$order_fields = array(
		'id' => array(
			'field' => 'id',
			'title' => '订单ID',
	...
	if(!empty($list)) {
		...
		$goods_temp = pdo_fetchall('select * from ' . tablename('tiny_wmall2_order_stat') . " where uniacid = :uniacid and oid in ({$oid_str})", array(':uniacid' => $_W['uniacid']));
			...
			foreach($all_fields as $key => $li) {
				...
				if(in_array($field, array_keys($order_fields))) {
					if($field == 'sid') {
						...
						$log = pdo_fetch('select * from ' . tablename('tiny_wmall2_order_status_log') . ' where uniacid = :uniacid and oid = :oid order by id desc', array(':uniacid' => $_W['uniacid'], ':oid' => $row['id']));
