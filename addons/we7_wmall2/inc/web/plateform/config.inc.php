...
if(empty($config['id'])) {
	...
	pdo_insert('tiny_wmall2_config', $init_config);
}
...
if($op == 'basic') {
	if(checksubmit('submit')) {
		...
		if(!empty($config['id'])) {
			pdo_update('tiny_wmall2_config', $data, array('uniacid' => $_W['uniacid']));
		} else {
			pdo_insert('tiny_wmall2_config', $data);
		}
		message('设置参数成功', referer(), 'success');
	}
	$stores = pdo_getall('tiny_wmall2_store', array('uniacid' => $_W['uniacid']));
...
if($op == 'order') {
	...
	if(checksubmit('submit')) {
		...
		pdo_update('tiny_wmall2_config', array('credit' => iserializer($credit)), array('uniacid' => $_W['uniacid']));
...
if($op == 'deliveryer') {
	...
	if(checksubmit()) {
		...
		if(!empty($config['id'])) {
			pdo_update('tiny_wmall2_delivery_config', $data, array('uniacid' => $_W['uniacid']));
		} else {
			pdo_insert('tiny_wmall2_delivery_config', $data);
		}
...
if($op == 'store') {
	...
	if(checksubmit('submit')) {
		...
		pdo_update('tiny_wmall2_config', array('store' => iserializer($store)) , array('uniacid' => $_W['uniacid']));
		...
		if($delivery_sync == 1) {
			pdo_update('tiny_wmall2_store_account', array('delivery_type' => $data['delivery_type'], 'delivery_price' => $data['plateform_delivery_fee']), array('uniacid' => $_W['uniacid']));
			pdo_update('tiny_wmall2_store', array('delivery_mode' => $data['delivery_type'], 'delivery_price' => $data['plateform_delivery_fee'], 'delivery_free_price' => 0), array('uniacid' => $_W['uniacid']));
			pdo_query('update ' .  tablename('tiny_wmall2_store_deliveryer') . ' set delivery_type = :delivery_type where uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':delivery_type' => $data['delivery_type']));
		}
