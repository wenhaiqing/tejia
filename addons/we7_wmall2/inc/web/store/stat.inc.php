...
global $_W, $_GPC;
$_W['page']['title'] = '订单统计-' . $_W['we7_wmall2']['config']['title'];
mload()->model('store');
...
if($op == 'list') {
	...
	$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall2_order') .  $condition, $params);
	$data = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_order') . $condition, $params);
	$total = array();
	if(!empty($data)) {
		foreach($data as &$da) {
...
if($op == 'order_num') {
	...
	if($_W['isajax'] && $_W['ispost']) {
		...
		$data = pdo_fetchall("SELECT * FROM " . tablename('tiny_wmall2_order') . 'WHERE uniacid = :uniacid AND sid = :sid and status = 5 and is_pay = 1 and addtime >= :starttime and addtime <= :endtime', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':starttime' => $start, 'endtime' => $end));
		foreach($data as $da) {
...
if($op == 'day') {
	...
	$data = array();
	$orders = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_order') . " WHERE uniacid = :aid AND sid = :sid AND is_pay = 1 and status = 5 and addtime >= :start AND addtime < :end", array(':sid' => $sid, ':aid' => $_W['uniacid'], ':start' => $starttime, ':end' => $endtime), 'id');
	$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall2_order') . " WHERE uniacid = :aid AND sid = :sid AND is_pay = 1 and status = 5  and addtime >= :start AND addtime < :end", array(':sid' => $sid, ':aid' => $_W['uniacid'], ':start' => $starttime, ':end' => $endtime));
	if(!empty($orders)) {
		$str = implode(',', array_keys($orders));
		$data = pdo_fetchall('SELECT *,SUM(goods_num) AS num, SUM(goods_price) AS price FROM ' . tablename('tiny_wmall2_order_stat') . " WHERE uniacid = :aid AND sid = :sid AND oid IN ({$str}) GROUP BY goods_id" . $order_by, array(':aid' => $_W['uniacid'], ':sid' => $sid), 'goods_id');
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall2_order_stat') . " WHERE uniacid = :aid AND sid = :sid AND oid IN ({$str})", array(':aid' => $_W['uniacid'], ':sid' => $sid));
		$price = pdo_fetchcolumn('SELECT SUM(final_fee) FROM ' . tablename('tiny_wmall2_order') . " WHERE uniacid = :aid AND sid = :sid AND id IN ({$str})", array(':aid' => $_W['uniacid'], ':sid' => $sid));
	}
	if(!empty($orders)) {
