...
global $_W, $_GPC;
$_W['page']['title'] = '收入/提现-' . $_W['we7_wmall2']['config']['title'];
mload()->model('coupon');
$store = store_check();
$sid = $store['id'];
$do = 'trade';
...
if($op == 'index') {
	...
	$stat['week_total'] = floatval(pdo_fetchcolumn('select sum(final_fee) from ' . tablename('tiny_wmall2_order_current_log') . ' where uniacid = :uniacid and sid = :sid and trade_status = 1 and addtime >= :addtime', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':addtime' => $addtime)));
	$stat['wait_total'] = floatval(pdo_fetchcolumn('select sum(final_fee) from ' . tablename('tiny_wmall2_order_current_log') . ' where uniacid = :uniacid and sid = :sid and trade_status = 2', array(':uniacid' => $_W['uniacid'], ':sid' => $sid)));
	$records = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_order_current_log') . 'WHERE uniacid = :uniacid AND sid = :sid ORDER BY id DESC LIMIT 30', array(':uniacid' => $_W['uniacid'], ':sid' => $sid));
...
if($op == 'inout') {
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall2_store_current_log') .  $condition, $params);
	$records = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_store_current_log') . $condition . ' ORDER BY id DESC LIMIT '.($pindex - 1) * $psize.','.$psize, $params);
...
if($op == 'getcash') {
	...
	if(checksubmit()) {
		...
		pdo_insert('tiny_wmall2_store_getcash_log', $data);
