...
$stores = pdo_fetchall('select id, uniacid, title from ' . tablename('tiny_wmall2_store') . ' where 1 order by id asc');
$i = 0;
if(!empty($stores)) {
	$settle_config = sys_settle_config($store['uniacid']);
	$delivery_config = sys_delivery_config($store['uniacid']);
	foreach($stores as $store) {
		$account = pdo_get('tiny_wmall2_store_account', array('sid' => $store['id']));
		if(empty($account)) {
			$store_account = array(
