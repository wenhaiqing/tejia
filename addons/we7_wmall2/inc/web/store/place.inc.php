...
global $_W, $_GPC;
$_W['page']['title'] = '后台下单-' . $_W['we7_wmall2']['config']['title'];
...
if($op == 'index') {
	...
	$goods = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_goods') . ' WHERE uniacid = :aid AND sid = :sid AND status = 1 ORDER BY displayorder DESC, id ASC', array(':aid' => $_W['uniacid'], ':sid' => $sid));
...
if($op == 'post') {
	...
	if(!empty($goods)) {
		...
		$goods_info = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_goods') ." WHERE uniacid = :aid AND sid = :sid AND id IN ({$goods})", array(':aid' => $_W['uniacid'], ':sid' => $sid), 'id');
	}
	$cart = array();
	$goods = array();
		...
		if(!$goods_info[$v]['is_options']) {
			...
			foreach($data['good']['options'] as $val) {
				...
				if($option_id > 0 && $option_num > 0) {
					$option = pdo_get('tiny_wmall2_goods_options', array('uniacid' => $_W['uniacid'], 'id' => $option_id));
