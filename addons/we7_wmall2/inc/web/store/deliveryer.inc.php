...
global $_W, $_GPC;
$_W['page']['title'] = '配送员管理-' . $_W['we7_wmall2']['config']['title'];
mload()->model('store');
...
if($op == 'add') {
	if($_W['isajax']) {
		...
		$is_exist = pdo_get('tiny_wmall2_store_deliveryer', array('uniacid' => $_W['uniacid'], 'deliveryer_id' => $deliveryer['id'], 'sid' => $sid));
		if(!empty($is_exist)) {
...
if($op == 'list') {
	$data = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_store_deliveryer') . ' WHERE uniacid = :uniacid AND sid = :sid', array(':uniacid' => $_W['uniacid'], ':sid' => $sid));
	if(!empty($data)) {
...
if($op == 'del') {
	$id = intval($_GPC['id']);
	pdo_delete('tiny_wmall2_store_deliveryer', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $id));
...
if($op == 'stat') {
	...
	if($_W['isajax'] && $_W['ispost']) {
		...
		$data = pdo_fetchall("SELECT * FROM " . tablename('tiny_wmall2_order') . 'WHERE uniacid = :uniacid AND sid = :sid AND deliveryer_id = :deliveryer_id and status = 5', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':deliveryer_id' => $id));
		foreach($data as $da) {
