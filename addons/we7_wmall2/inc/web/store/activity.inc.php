...
global $_W, $_GPC;
$_W['page']['title'] = '营销活动-' . $_W['we7_wmall2']['config']['title'];
mload()->model('store');
$store = store_check();
$sid = $store['id'];
$do = 'activity';
...
if($op == 'first_order') {
	if(checksubmit()) {
		...
		pdo_update('tiny_wmall2_store', array('first_order_status' => $first_order_status), array('uniacid' => $_W['uniacid'], 'id' => $sid));
		pdo_update('tiny_wmall2_store_activity', array('first_order_status' => $first_order_status, 'first_order_data' => iserializer($first_order_data)), array('uniacid' => $_W['uniacid'], 'sid' => $sid));
...
if($op == 'discount') {
	if(checksubmit()) {
		...
		pdo_update('tiny_wmall2_store', array('discount_status' => $discount_status), array('uniacid' => $_W['uniacid'], 'id' => $sid));
		pdo_update('tiny_wmall2_store_activity', array('discount_status' => $discount_status, 'discount_data' => iserializer($discount_data)), array('uniacid' => $_W['uniacid'], 'sid' => $sid));
...
if($op == 'grant') {
	if(checksubmit()) {
		...
		pdo_update('tiny_wmall2_store', array('grant_status' => $grant_status), array('uniacid' => $_W['uniacid'], 'id' => $sid));
		pdo_update('tiny_wmall2_store_activity', array('grant_status' => $grant_status, 'grant_data' => iserializer($grant_data)), array('uniacid' => $_W['uniacid'], 'sid' => $sid));
...
if($op == 'amount') {
	if(checksubmit()) {
		...
		if(!empty($_GPC['categorys'])) {
			...
			$goods = pdo_fetchall('select id,cid from ' . tablename('tiny_wmall2_goods') . " where uniacid = :uniacid and sid = :sid and cid in ({$cids})", array(':uniacid' => $_W['uniacid'], ':sid' => $sid), 'id');
		}
		$update = array(
		...
		pdo_update('tiny_wmall2_store', array('amount_status' => $amount_status), array('uniacid' => $_W['uniacid'], 'id' => $sid));
		pdo_update('tiny_wmall2_store_activity', $update, array('uniacid' => $_W['uniacid'], 'sid' => $sid));
	...
	$categorys = pdo_getall('tiny_wmall2_goods_category', array('uniacid' => $_W['uniacid'], 'sid' => $sid));
}
