...
global $_W, $_GPC;
$_W['page']['title'] = '门店列表-' . $_W['we7_wmall2']['config']['title'];
mload()->model('store');
...
if($_GPC['__sid'] > 0) {
	$csotre = pdo_get('tiny_wmall2_store', array('uniacid' => $_W['uniacid'], 'id' => $_GPC['__sid']), array('title'));
	if(empty($csotre)) {
		$_GPC['__sid'] = 0;
	}
}
if($_W['role'] == 'operator') {
	$id = intval($_GPC['id']);
	if($_W['we7_wmall2']['store']['id'] != $id && $id > 0) {
...
if($op == 'post') {
	...
	if($id) {
		...
		$delivery_times = pdo_getall('tiny_wmall2_store_delivery_times', array('uniacid' => $_W['uniacid'], 'sid' => $id));
		$sys_url = murl('entry', array('m' => 'we7_wmall2', 'do' => 'store', 'sid' => $item['id']), true, true);
	...
	if(checksubmit('submit')) {
		...
		if($id) {
			pdo_update('tiny_wmall2_store', $data, array('uniacid' => $_W['uniacid'], 'id' => $id));
			$sid = $id;
		} else {
			$data['uniacid'] = $_W['uniacid'];
			$data['addtime'] = TIMESTAMP;
			pdo_insert('tiny_wmall2_store', $data);
			$sid = pdo_insertid();
			//添加门店账户数据
			...
			$idd = pdo_insert('tiny_wmall2_store_account', $store_account);
			$update = array(
			...
			pdo_update('tiny_wmall2_store', $update, array('uniacid' => $_W['uniacid'], 'id' => $id));
		}
		$timeids = array(0);
		if(!empty($_GPC['starttime'])) {
			foreach($_GPC['starttime'] as $key => $val) {
				...
				if(!empty($start) && !empty($end)) {
					...
					if($id > 0) {
						pdo_update('tiny_wmall2_store_delivery_times', $data, array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $id));
					} else {
						pdo_insert('tiny_wmall2_store_delivery_times', $data);
						$id = pdo_insertid();
					}
					$timeids[] = $id;
				}
		...
		pdo_query('delete from ' . tablename('tiny_wmall2_store_delivery_times') . " where uniacid = :uniacid and sid = :sid and id not in ({$timeids})", array(':uniacid' => $_W['uniacid'], ':sid' => $sid));
	...
	$pay = $_W['we7_wmall2']['config']['payment'];
	if(empty($pay)) {
...
if($op == 'list') {
	...
	if($_W['role'] == 'operator') {
		$condition .= " AND id = :id";
		$params[':id'] = $_W['we7_wmall2']['store']['id'];
	}
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall2_store') . ' WHERE ' . $condition, $params);
	$lists = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_store') . ' WHERE ' . $condition . ' ORDER BY displayorder DESC LIMIT '.($pindex - 1) * $psize.','.$psize, $params);
	$pager = pagination($total, $pindex, $psize);
	if(!empty($lists)) {
		foreach($lists as &$li) {
			$li['cid'] = explode('|', $li['cid']);
			$li['address'] = str_replace('+', ' ', $li['district']) . ' ' . $li['address'];
			$li['sys_url'] = murl('entry', array('m' => 'we7_wmall2', 'do' => 'store', 'sid' => $li['id']), true, true);
			$li['wechat_qrcode'] = (array)iunserializer($li['wechat_qrcode']);
			$li['wechat_url'] = $li['wechat_qrcode']['url'];
			$li['user'] = pdo_get('tiny_wmall2_clerk', array('uniacid' => $_W['uniacid'], 'sid' => $li['id'], 'role' => 'manager'));
		}
	}
	$categorys = store_fetchall_category();
...
if($op == 'template') {
	...
	pdo_update('tiny_wmall2_store', array('template' => $template), array('uniacid' => $_W['uniacid'], 'id' => $sid));
...
if($op == 'status') {
	if($_W['isajax']) {
		...
		pdo_update('tiny_wmall2_store', array('status' => $status), array('uniacid' => $_W['uniacid'], 'id' => $sid));
		exit();
	}
}
...
if($op == 'recommend') {
	if($_W['isajax']) {
		...
		pdo_update('tiny_wmall2_store', array('is_recommend' => $recommend), array('uniacid' => $_W['uniacid'], 'id' => $sid));
		exit();
	}
}
...
if($op == 'is_in_business') {
	if($_W['isajax']) {
		...
		pdo_update('tiny_wmall2_store', array('is_in_business' => $is_in_business), array('uniacid' => $_W['uniacid'], 'id' => $sid));
		exit();
	}
}
...
if($op == 'edit') {
	...
	pdo_update('tiny_wmall2_store', array($type => $value), array('uniacid' => $_W['uniacid'], 'id' => $sid));
	exit('success');
}
...
if($op == 'copy') {
	...
	pdo_insert('tiny_wmall2_store_account', $store_account);

	//门店配送时间
	$times = pdo_getall('tiny_wmall2_store_delivery_times', array('uniacid' => $_W['uniacid'], 'sid' => $sid));
	if(!empty($times)) {
		foreach($times as $time) {
			unset($time['id']);
			$time['sid'] = $store_id;
			pdo_insert('tiny_wmall2_store_delivery_times', $time);
		}
	}

	//配送员数据
	$deliveryers = pdo_getall('tiny_wmall2_store_deliveryer', array('uniacid' => $_W['uniacid'], 'sid' => $sid));
	if(!empty($deliveryers)) {
		foreach($deliveryers as $deliveryer) {
			unset($deliveryer['id']);
			$deliveryer['sid'] = $store_id;
			pdo_insert('tiny_wmall2_store_deliveryer', $deliveryer);
		}
	}
	
	//门店优惠活动
	$activity = pdo_get('tiny_wmall2_store_activity', array('uniacid' => $_W['uniacid'], 'sid' => $sid));
	if(!empty($activity)) {
		unset($activity['id']);
		$activity['sid'] = $store_id;
		pdo_insert('tiny_wmall2_store_activity', $activity);
	}

	//复制菜品分类
	$goods_categorys = pdo_getall('tiny_wmall2_goods_category', array('uniacid' => $_W['uniacid'], 'sid' => $sid));
	if(!empty($goods_categorys)) {
		foreach($goods_categorys as $category) {
			$cid = $category['id'];
			unset($category['id']);
			$category['sid'] = $store_id;
			pdo_insert('tiny_wmall2_goods_category', $category);
			$category_id = pdo_insertid();
			$goods = pdo_getall('tiny_wmall2_goods', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'cid' => $cid));
			if(!empty($goods)) {
				foreach($goods as $good) {
					...
					pdo_insert('tiny_wmall2_goods', $good);
					$new_goods_id = pdo_insertid();
					if($good['is_options'] == 1) {
						$options = pdo_getall('tiny_wmall2_goods_options', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'goods_id' => $goods_id));
						if(!empty($options)) {
							foreach($options as $option) {
								unset($option['id']);
								$option['sid'] = $store_id;
								$option['goods_id'] = $new_goods_id;
								pdo_insert('tiny_wmall2_goods_options', $option);
							}
						}
					}
				}
	...
	//复制桌台类型
	$table_categorys = pdo_getall('tiny_wmall2_tables_category', array('uniacid' => $_W['uniacid'], 'sid' => $sid));
	if(!empty($table_categorys)) {
		foreach($table_categorys as $category) {
			$cid = $category['id'];
			unset($category['id']);
			$category['sid'] = $store_id;
			pdo_insert('tiny_wmall2_tables_category', $category);
			$category_id = pdo_insertid();
			$tables = pdo_getall('tiny_wmall2_tables', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'cid' => $cid));
			if(!empty($tables)) {
				foreach($tables as $table) {
					...
					pdo_insert('tiny_wmall2_tables', $table);
				}
			}
			//复制预定
			$reserves = pdo_getall('tiny_wmall2_reserve', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'table_cid' => $cid));
			if(!empty($reserves)) {
				foreach($reserves as $reserve) {
					unset($reserve['id']);
					$reserve['sid'] = $store_id;
					$reserve['table_cid'] = $category_id;
					pdo_insert('tiny_wmall2_reserve', $reserve);
				}
			}
		}
	}
	...
	//复制排号
	$assigns = pdo_getall('tiny_wmall2_assign_queue', array('uniacid' => $_W['uniacid'], 'sid' => $sid));
	if(!empty($assigns)) {
		foreach($assigns as $assign) {
			unset($assign['id']);
			$assign['sid'] = $store_id;
			pdo_insert('tiny_wmall2_assign_queue', $assign);
		}
	}
...
if($op == 'del') {
	$id = intval($_GPC['id']);
	pdo_delete('tiny_wmall2_store', array('uniacid' => $_W['uniacid'], 'id' => $id));
	$tables = array(
		'tiny_wmall2_activity_coupon',
		'tiny_wmall2_activity_coupon_grant_log',
		'tiny_wmall2_activity_coupon_record',
		'tiny_wmall2_clerk',
		'tiny_wmall2_goods',
		'tiny_wmall2_goods_category',
		'tiny_wmall2_goods_options',
		'tiny_wmall2_order',
		'tiny_wmall2_order_cart',
		'tiny_wmall2_order_comment',
		'tiny_wmall2_order_current_log',
		'tiny_wmall2_order_discount',
		'tiny_wmall2_order_print_log',
		'tiny_wmall2_order_refund_log',
		'tiny_wmall2_order_stat',
		'tiny_wmall2_order_status_log',
		'tiny_wmall2_printer',
		'tiny_wmall2_reply',
		'tiny_wmall2_sms_send_log',
		'tiny_wmall2_store_account',
		'tiny_wmall2_store_activity',
		'tiny_wmall2_store_current_log',
		'tiny_wmall2_store_delivery_times',
		'tiny_wmall2_store_favorite',
		'tiny_wmall2_store_getcash_log',
		'tiny_wmall2_store_deliveryer',
		'tiny_wmall2_store_members',
		'tiny_wmall2_tables',
		'tiny_wmall2_tables_category',
		'tiny_wmall2_reserve',
		'tiny_wmall2_assign_board',
		'tiny_wmall2_assign_queue',
	);
	foreach($tables as $table) {
