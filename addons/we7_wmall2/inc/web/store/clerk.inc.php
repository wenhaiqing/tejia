...
global $_W, $_GPC;
$_W['page']['title'] = '店员管理-' . $_W['we7_wmall2']['config']['title'];
mload()->model('store');
mload()->model('clerk');
...
if($op == 'post') {
	...
	if($_W['ispost']) {
		...
		$is_exist = pdo_get('tiny_wmall2_clerk', array('uniacid' => $_W['uniacid'], 'title' => $insert['title']));
		...
		if($role == 'manager' || $clerk['role'] == 'manager') {
			$manager = pdo_get('tiny_wmall2_clerk', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'role' => 'manager'));
			if(empty($manager)) {
				$insert['role'] = 'manager';
			}
		} else {
		...
		if($id > 0) {
			pdo_update('tiny_wmall2_clerk', $insert, array('uniacid' => $_W['uniacid'], 'id' => $id));
		} else {
			$insert['addtime'] = TIMESTAMP;
			pdo_insert('tiny_wmall2_clerk', $insert);
		}
		exit('success');
	}
...
if($op == 'list') {
	$data = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_clerk') . ' WHERE uniacid = :aid AND sid = :id', array(':aid' => $_W['uniacid'], ':id' => $sid));
	include $this->template('store/clerk');
}
...
if($op == 'del') {
	...
	if($clerk['role'] != 'manager') {
		pdo_delete('tiny_wmall2_clerk', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'id' => $id));
	}
