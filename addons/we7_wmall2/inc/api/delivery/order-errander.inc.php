...
if($op == 'list') {
	...
	$min_id = intval(pdo_fetchcolumn('SELECT min(id) as min_id FROM ' . tablename('tiny_wmall2_errander_order') . $condition , $params));
	$orders = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_errander_order') . $condition . ' order by id desc limit 15', $params, 'id');
	$min = $max = 0;
	if(!empty($orders)) {
