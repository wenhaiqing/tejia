...
if($op == 'getcash_submit') {
	...
	pdo_insert('tiny_wmall2_deliveryer_getcash_log', $data);
...
if($op == 'getcash_detail') {
	$id = intval($_GPC['id']);
	$log = pdo_get('tiny_wmall2_deliveryer_getcash_log', array('uniacid' => $_W['uniacid'], 'id' => $id, 'deliveryer_id' => $deliveryer['id']));
	if(empty($log)) {
...
if($op == 'inout') {
	...
	$min_id = intval(pdo_fetchcolumn('SELECT min(id) as min_id FROM ' . tablename('tiny_wmall2_deliveryer_current_log') . $condition , $params));
	$records = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall2_deliveryer_current_log') . $condition . ' ORDER BY id DESC LIMIT 20', $params, 'id');
	$min = $max = 0;
	if(!empty($records)) {
		$trade_types = array(
			'1' => '配送费入账',
