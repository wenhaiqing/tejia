...
function errander_category_fetch($id) {
	global $_W;
	$category = pdo_get('tiny_wmall2_errander_category', array('uniacid' => $_W['uniacid'], 'id' => $id));
	if(!empty($category)) {
...
function errander_order_fetch($id) {
	global $_W;
	$id = intval($id);
	$order = pdo_fetch('SELECT * FROM ' . tablename('tiny_wmall2_errander_order') . ' WHERE uniacid = :aid AND id = :id', array(':aid' => $_W['uniacid'], ':id' => $id));
	if(empty($order)) {
		return false;
	}
	...
	$order['category'] = pdo_get('tiny_wmall2_errander_category', array('uniacid' => $_W['uniacid'], 'id' => $order['order_cid']), array('id', 'title', 'thumb'));
...
function errander_order_fetch_status_log($id) {
	global $_W;
	$data = pdo_fetchall("SELECT * FROM " . tablename('tiny_wmall2_errander_order_status_log') . ' WHERE uniacid = :uniacid and oid = :oid order by id asc', array(':uniacid' => $_W['uniacid'], ':oid' => $id), 'id');
	return $data;
}
...
function errander_order_insert_status_log($id, $type, $note = '') {
	...
	$config = $_W['we7_wmall2']['config']['errander'];
	mload()->model('store');
	...
	pdo_insert('tiny_wmall2_errander_order_status_log', $data);
...
function errander_order_status_notice($id, $status, $note = '') {
	...
	if(!empty($order['openid'])) {
		...
		$url = murl('entry', array('do' => 'errander-order', 'm' => 'we7_wmall2', 'op' => 'detail', 'id' => $order['id']), true, true);
		$remark = implode("\n", $remark);
...
function errander_order_insert_refund_log($id, $type, $note = '') {
	...
	$notes = array(
		...
		'handel' => array(
			'status' => 2,
			'title' => "{$_W['we7_wmall2']['config']['title']}接受退款申请",
			'note' => ''
		),
		'success' => array(
			'status' => 3,
...
function errander_order_fetch_refund_status_log($id) {
	global $_W;
	$data = pdo_fetchall("SELECT * FROM " . tablename('tiny_wmall2_order_refund_log') . ' WHERE uniacid = :uniacid and oid = :oid and order_type = :order_type order by id asc', array(':uniacid' => $_W['uniacid'], ':oid' => $id, ':order_type' => 'errander'), 'id');
	return $data;
}
...
function errander_order_deliveryer_notice($id, $type, $deliveryer_id = 0, $note = '') {
	...
	if($type == 'new_delivery') {
		...
		$remark = implode("\n", $remark);
		$url = murl('entry', array('do' => 'dyerrander', 'm' => 'we7_wmall2', 'op' => 'detail', 'id' => $order['id']), true, true);
		//手机端推送
		...
		$remark = implode("\n", $remark);
		$url = $_W['siteroot'] . 'app' . ltrim(murl('entry', array('do' => 'dyerrander-order', 'm' => 'we7_wmall2', 'op' => 'list')), '.');
		//手机端推送
...
function errander_order_assign_deliveryer($order_id, $deliveryer_id, $update_deliveryer = false) {
	...
	$permission = pdo_get('tiny_wmall2_store_deliveryer', array('uniacid' => $_W['uniacid'], 'sid' => 0, 'deliveryer_id' => $deliveryer_id));
	if(empty($permission)) {
	...
	pdo_update('tiny_wmall2_errander_order', $update, array('uniacid' => $_W['uniacid'], 'id' => $order_id));
...
function errander_order_manager_notice($order_id, $type, $note = '') {
	...
	$remark = implode("\n", $remark);
	$send = tpl_format($title, $order['order_sn'], $order['status_cn'], $remark);
	$status = $acc->sendTplNotice($maneger['openid'], $_W['we7_wmall2']['config']['public_tpl'], $send);
	return $status;
}
...
function errander_order_status_update($id, $type, $extra = array()) {
	...
	if($type == 'dispatch') {
		...
		if(!$order['is_pay'] || $order['final_fee'] <= 0) {
			pdo_update('tiny_wmall2_errander_order', array('status' => 4), array('uniacid' => $_W['uniacid'], 'id' => $id));
			...
			pdo_update('tiny_wmall2_errander_order', $update, array('uniacid' => $_W['uniacid'], 'id' => $id));
			errander_order_insert_status_log($id, 'cancel');
		...
		$deliveryer = pdo_get('tiny_wmall2_deliveryer', array('uniacid' => $_W['uniacid'], 'id' => $extra['deliveryer_id']));
		if(empty($deliveryer)) {
			return error(-1, '配送员不存在');
		}
		$update = array(
...
function errander_order_refund_notice($order_id, $type, $note = '') {
	...
	if($type == 'apply') {
		...
		if(!empty($order['openid'])) {
			...
			$send = sys_wechat_tpl_format($params);
			$status = $acc->sendTplNotice($order['openid'], $_W['we7_wmall2']['config']['notice']['refund_tpl'], $send);
		}
	} elseif($type == 'success') {
		if(!empty($order['openid'])) {
...
function errander_order_begin_payrefund($id) {
	...
	if($order['pay_type'] == 'credit') {
		if($order['uid'] > 0) {
			$log = array(
				$order['uid'],
				"外送模块(we7_wmall2)订单退款, 订单号:{$order['id']}, 退款金额:{$order['final_fee']}元",
				'we7_wmall2'
			);
			load()->model('mc');
			...
			pdo_update('tiny_wmall2_errander_order', $update, array('uniacid' => $_W['uniacid'], 'id' => $order['id']));
