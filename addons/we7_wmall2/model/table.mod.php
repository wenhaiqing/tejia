...
function table_category_fetchall($sid) {
	global $_W;
	$data = pdo_getall('tiny_wmall2_tables_category', array('uniacid' => $_W['uniacid'], 'sid' => $sid), array(), 'id');
	return $data;
}
...
function table_fetch($table_id) {
	global $_W;
	$table = pdo_get('tiny_wmall2_tables', array('uniacid' => $_W['uniacid'], 'id' => $table_id));
	if(!empty($table)) {
		$table['category'] = pdo_get('tiny_wmall2_tables_category', array('uniacid' => $_W['uniacid'], 'id' => $table['cid']));
	}
	return $table;
}
...
function table_order_update($table_id, $order_id, $status) {
	global $_W;
	$status = pdo_update('tiny_wmall2_tables', array('order_id' => $order_id, 'status' => $status), array('uniacid' => $_W['uniacid'], 'id' => $table_id));
	return $status;
}
...
function assign_board_fetch($id) {
	global $_W;
	$board = pdo_get('tiny_wmall2_assign_board', array('uniacid' => $_W['uniacid'], 'id' => $id));
	return $board;
}
...
function assign_queue_fetch($id) {
	global $_W;
	$queue = pdo_get('tiny_wmall2_assign_queue', array('uniacid' => $_W['uniacid'], 'id' => $id));
	return $queue;
}
...
function assign_notice($sid, $id, $status) {
	...
	if(!empty($config['notice']['assign_tpl'])) {
		...
		$url = murl('entry', array('m' => 'we7_wmall2', 'do' => 'assign', 'sid' => $sid, 'op' => 'goods'), true, true);
		$wait_count = pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall2_assign_board') . ' where uniacid = :uniacid and sid = :sid and status = 1 and id < :id and queue_id = :queue_id', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':queue_id' => $board['queue_id'], ':id' => $board['id']));
...
function assign_notice_queue($board_id, $queue_id) {
	...
	if(!empty($queue) && $queue['notify_num'] > 0) {
		$boards = pdo_fetchall('select * from ' . tablename('tiny_wmall2_assign_board') . ' where uniacid = :uniacid and sid = :sid and queue_id = :queue_id and status = 1 and id > :id limit ' . $queue['notify_num'], array(':uniacid' => $_W['uniacid'], ':sid' => $queue['sid'], ':queue_id' => $queue_id, ':id' => $board_id));
		if(!empty($boards)) {
