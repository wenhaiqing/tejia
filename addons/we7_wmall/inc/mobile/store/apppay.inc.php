...
$whq_paytype = trim($_GPC['paytype']);
$whq_uid = intval($_GPC['uid']);
if(empty($id) || empty($type)) {
	returnjson('参数错误', '', 'error');
}
$tables = array(
...
if(!empty($order['is_pay'])) {
	returnjson('该订单已付款', '', 'info');
}
if($type == 'order' && $order['status'] == 6) {
	returnjson('订单已取消',$this->createMobileUrl('order'), 'info');
}
if($order['final_fee'] == 0) {
	returnjson('订单支付成功', '', 'success');
}
...
if(empty($record)) {
	...
	if($record['status'] == 1) {
		returnjson('该订单已支付,请勿重复支付', '', 'error');
	}
}
...
//if($pay_type && !$_GPC['type'] && $available_pay_types[$pay_type] == 1) {
	...
	// header('location:' . murl("mc/cash/{$whq_paytype}" , array('params' => $params)));
	// die;
// } else {
// 	if(empty($available_pay_types)) {
// 		returnjson('没有有效的支付方式, 请联系网站管理员.');
// 	}
// 	if(empty($uid)) {
